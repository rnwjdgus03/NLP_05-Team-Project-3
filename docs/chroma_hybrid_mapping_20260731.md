...
| | 건수 | |
|---|---|---|
| 확정 | 12 (14%) | READY 또는 PROVISIONAL |
| 확인 불가 | 24 (27%) | 기간이 기사일 이후 15 · 좌표는 맞고 데이터 없음 4 · 기간 불명 5 |
| 시스템 한계 | 52 (59%) | 후보 비결정 24 · 단위 확인 불가 18 · 좌표 오류 10 |

- **전체 기준 13.6%** — 사용자가 기사를 넣었을 때 실제로 답을 받을 확률
- **확인 가능 기준 18.8%** — 답할 수 있는 문제에서 시스템이 얼마나 하는가

집합이 103 → 88 로 줄어 비율이 올랐다. **이것은 개선이 아니다** —
출처 귀속 전파로 범위 밖 주장(한은 제출자료 등)을 걷어낸 결과다.
의미 있는 것은 **확정 건수가 12 로 유지됐다**는 점이다.
