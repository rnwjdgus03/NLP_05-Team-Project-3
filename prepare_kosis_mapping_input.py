...
def individual_product_price(row: dict, dimension: str) -> bool:
    ...
    if any(term in compact_indicator for term in _AGGREGATE_PRICE_TERMS):
        return False
    if any(term in compact_indicator for term in ("판매가", "출고가", "정가")):
        return True
    text = nz(row.get("claim_text"))
    ...
    return any(cue in between or cue in text for cue in ("에서", "으로", "인상", "인하"))
...
def normalize_row(row: dict) -> dict:
    ...
    canonical_unit = canonical_measurement_unit(row)
    dimension = unit_dimension(canonical_unit)
    relative_period, relative_status = recover_relative_measurement_period(row)
    recovery_row = dict(row)
    if "measurement_period" in row:
    ...
    contract_row = dict(recovery_row)
    if "measurement_period" in row:
        contract_row["measurement_period"] = recovered_period
    else:
        contract_row["period"] = recovered_period
    ...
    out["claim_indicator"] = claim_value(row, "claim_indicator", "indicator")
    out["claim_industry_or_item"] = claim_value(
        row, "claim_industry_or_item", "industry_or_item"
    )
    out["claim_period"] = claim_value(row, "claim_period", "period")
    out["claim_prd_se"] = claim_value(row, "claim_prd_se", "prd_se")
    out["raw_measurement_period"] = measurement_value(
        row, "measurement_period", "period"
    )
    out["indicator"] = measurement_value(row, "measurement_indicator", "indicator")
    out["obj_target_terms"] = merge_obj_target_terms(
        row.get("obj_target_terms"), explicit_employee_size_targets(row)
    )
    ...
    out["prd_se"] = measurement_value(row, "measurement_prd_se", "prd_se")
    out["period"], change_alignment_status = align_change_period(contract_row)
    out["period_alignment_status"] = "|".join(
        value for value in (relative_status, recovery_status, change_alignment_status) if value
    )
