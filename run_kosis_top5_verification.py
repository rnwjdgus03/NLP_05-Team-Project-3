...
def align_period(
    ...
    if requested in supported:
    ...
    elif len(target) == 4 and "Y" in supported:
        ...
        len(target_shape) == 6
        and "Q" in supported
        and target_shape[-2:] in {"01", "02", "03", "04"}
    ):
        api_prd_se, state = "Q", "period_shape_inference"
        target = normalize_api_period(target_raw, api_prd_se)
        previous = normalize_api_period(previous_raw, api_prd_se)
...
def _period_in_range(target: str, rows: Sequence[Mapping[str, Any]], prd_se: str) -> bool | None:
    matching = [row for row in rows if normalize_periodicity(row.get("prd_se")) == prd_se]
    ranges = [_text(row.get("range_text")) for row in matching if _text(row.get("range_text"))]
    if not target or not ranges:
        return None
    digits = [re.findall(r"(?:19|20)\d{2}(?:0[1-9]|1[0-2])?", value) for value in ranges]
    bounds = [part for part in digits if len(part) >= 2]
    if not bounds:
        return None
    comparable = target if len(target) == 6 else target[:4]
    return any(
        (start[:len(comparable)] <= comparable <= end[:len(comparable)])
        for start, *middle, end in bounds
    )
...
def postgres_preflight(
    ...
    repaired_axes = list(coordinate.get("axis_values") or [])
    ...
    valid = not (missing_axes or unknown_axes or invalid_values)
...
def block_unconfirmed_mismatch(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    value.update({
        ...
        "verdict_reason": (
            "정확한 표·ITEM·OBJ·기간·단위가 모두 확정되지 않아 "
...
def main() -> None:
    ...
    claims = {
        _text(row.get("claim_measurement_id") or row.get("claim_id")): row
    ...
    for claim_id, rows in sorted(by_claim.items()):
        ...
        claim_rows.append({
            ...
            "candidate_verdict_counts": dict(Counter(
                _text(row.get("verdict_code")) for row in rows
            )),
            "failure_stage_counts": dict(Counter(
                _text(row.get("verdict_stage")) or "unknown" for row in rows
            )),
