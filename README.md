...
Chroma-ready 기준 운영 기본값은 `--retrieval-mode hybrid`입니다. 재현성 또는 오프라인 비교가 필요할 때만 `--retrieval-mode lexical`을 명시합니다. `hybrid`는 임베딩 인덱스와 ML 의존성이 없을 때 즉시 실패하므로 Colab/GPU 환경에서 실행하는 것을 권장합니다. `--no-reranker`를 주면 임베딩과 규칙 검색만 결합합니다.
...
## Chroma-ready KOSIS 매핑 파이프라인 기준

이 브랜치의 기준 용어는 `verification_input_ready`와 `mapping_status=READY`를 분리한다.

| 단계 | 컬럼/상태 | 의미 | 최종 수치 검증 여부 |
|---|---|---|---|
| 입력 prefilter | `verification_input_ready=Y` | KOSIS 검증을 시도해볼 수 있는 measurement 입력 | 아직 아님 |
| 매핑 검증 | `mapping_status=READY` | KOSIS 공식 메타와 실제 API 응답의 `ITM_ID`, `C1~C8`, 기간, 단위가 exact match된 후보 | 가능 |
| 매핑 보류 | `PROVISIONAL`, `NEEDS_CONFIRMATION` | 후보 점수는 높지만 exact match 또는 의미 확정이 부족함 | 불가 |
| 매핑 실패 | `NO_MATCH`, `INVALID_COMBINATION`, `API_ERROR` 등 | 표/좌표/API 응답이 검증 기준을 통과하지 못함 | 불가 |

구명칭 `in_ready` 또는 `mapping_eligible`은 최종 READY가 아니다. 호환을 위해 일부 파일에 남아 있을 수 있지만, 발표·문서·새 산출물에서는 `verification_input_ready`를 사용한다.

### 최종 파이프라인

```text
뉴스 claim
→ verification_input_ready prefilter
→ KOSIS 관련 claim만 통과
→ 표 후보 keyword + embedding hybrid search Top 20
→ table reranker Top 5
→ period-aware table routing
→ KOSIS 공식 메타 조회
→ 표 내부 coordinate 검색
→ ChromaDB + BGE-M3 dense + lexical + reranker coordinate Top 20
→ API 직전 후보 Top 5
→ KOSIS API 실제 호출
→ 응답 ITM_ID/C1~C8/기간/단위 exact match
→ READY / PROVISIONAL / NEEDS_CONFIRMATION / NO_MATCH
→ READY만 최종 수치 검증
```
...
| 파라미터 | 기본값 | 의미 |
|---|---:|---|
| `--top-tables` | 20 | 표 후보 검색 Top-K |
| `--top-rank-for-meta` | 5 | rerank 후 메타 조회 대상 표 Top-K |
| `--chroma-validation-top-k` | 20 | coordinate 검색 후보 Top-K |
| `--max-combinations` / `--api-candidate-top-k` | 5 | API 직전 후보 Top-K |
| debug/recall 모드 `--max-combinations` | 20 | 놓친 후보 원인 분석용 확장 모드 |

### period-aware table routing

KOSIS는 같은 현실 지표라도 기간에 따라 통계표 이름이나 `tbl_id`가 바뀔 수 있다.
예를 들어 로봇 수입 계열은 연도별로 다음처럼 갈라질 수 있다.

```text
2007-2012: 로봇단품 및 부품 수입현황
2013-2018: 로봇 단품 및 부품 국가별 수입 현황
2019-2024: 로봇산업 수입 현황
```

이 경우 단순히 `indicator=로봇 수입` 하나로 하나의 통계표만 고르면 기간이 맞아도
`tbl_id`가 틀어질 수 있다. 그래서 `data/claims/kosis_period_table_routes.csv`에
기간별 라우팅 규칙을 둔다.

라우팅 규칙은 최종 검증이 아니다. 역할은 다음 하나다.

```text
measurement indicator + item + period
→ 해당 기간에 맞는 KOSIS 통계표 후보를 검색 결과 상단에 주입
→ 이후 공식 메타 + KOSIS API exact match로 READY 여부 판단
```

즉, 라우팅된 후보라도 KOSIS API 응답의 `ITM_ID`, `C1~C8`, 기간, 단위가 맞지 않으면
`mapping_status=READY`가 되지 않는다. 표 개편 전후 조사대상·분류·단위 동일성이
확정되지 않은 규칙은 `PROVISIONAL_PERIOD_ROUTED`로 남기고, 사람이 승인한 뒤
`CONFIRMED_PERIOD_ROUTED`로 올린다.

### prefilter 기준

`prepare_kosis_mapping_input.py`는 다음 조건을 기준으로 `verification_input_ready=Y/N`을 만든다.

- 수치값이 존재하고 숫자로 파싱 가능해야 한다.
- `measurement_indicator`, `measurement_period`, `measurement_prd_se`가 있어야 한다.
- `measurement_binding_source=hcx`로 measurement와 claim의 결합 근거가 있어야 한다.
- 목표값은 관측 통계가 아니므로 제외한다.
- 참고값은 주장과의 관계가 명확할 때만 통과한다.
- 단위가 표준화 가능해야 하고, 값 유형과 단위 차원이 충돌하면 제외한다.
- 순위값은 원자료 수준값과 직접 비교하지 않는다.
- 개인 사례, 전망/목표, 법령 조항, 일회성 사건, 설문 안내, 수사적 문장, 정책/제도 값은 제외하거나 확인 대상으로 둔다.

### READY 판정 기준

READY 기준은 완화하지 않는다. 임베딩, Chroma, reranker는 후보 순위를 좋게 만드는 도구일 뿐 READY를 직접 만들지 않는다. `mapping_status=READY`는 KOSIS API 응답에서 요청한 `ITM_ID`, `C1~C8`, 기간, 단위가 exact match될 때만 부여한다. `kosis_verify_claim_values.py`는 `mapping_status=READY`인 행만 최종 값 검증 대상으로 사용한다.
...
python run_kosis_measurement_pipeline.py \
  ...
  --table-index kosis_table_summary.csv \
  --out-dir outputs/kosis_mapping_ready_500_real_run \
  --retrieval-mode hybrid \
  --top-tables 20 \
  --top-rank-for-meta 5 \
  --chroma-coordinate-index data/indexes/kosis_chroma_coordinates_colab \
...
### 골드셋 전 단계 채점과 게이트 scope 수정 (2026-07-22)
...
| ② 게이트 | 재현율 P(ready\|verifiable) | 69.0% (scope 수정 전 59.5%) |
| ② 추출 | 필드 정확도(ready) | 87.9% |
| ③ 검색 | recall@5 | 62.5% |
| ④ 판정 | 정확도 | 31.8% |
...
**남은 과제**: (1) A팀 골드 기준 통일 — 기간·목표·조건값(usage=CONTEXT/CONDITION)은 `gold_verifiable=N`. (2) 코드북 override(검색 못 찾는 실적표), 무역수지 계산식, 품목 코드셋.

> 주의: 파일럿 표본이 24~33건 규모로 방향성 결론입니다. 골드 확장 시 재검증합니다.

### ITEM/OBJ 공식 메타·API exact match 검증 추가 (2026-07-23)

Poc의 결론은 `TBL_ID` 후보 검색에서는 lexical을 기본으로 쓰는 것입니다. 다만 `TBL_ID`를 찾은 뒤에도 어떤 `ITEM`과 `OBJ`를 선택해야 하는지는 별도 검증이 필요합니다. 이번 통합에서는 lexical 후보 검색 뒤에 **KOSIS 공식 메타데이터와 실제 데이터 API 응답으로 ITEM/OBJ 조합을 검증하는 단계**를 추가했습니다.

검증 흐름은 다음과 같습니다.

```text
lexical 기반 TBL_ID 후보 검색
→ 후보 TBL_ID의 KOSIS 공식 메타 조회
→ 공식 메타에 존재하는 ITEM/OBJ 코드만 후보로 구성
→ OBJ_ID_SN 기준 objL1~objL8 축 순서 보존
→ 실제 KOSIS 데이터 API 호출
→ 응답 ITM_ID/C1~C8이 요청 itmId/objL1~objL8과 exact match되는지 확인
→ 통과 후보만 mapping_status=READY
→ READY만 실제 수치 검증
```

`verification_input_ready=Y`와 `mapping_status=READY`는 서로 다릅니다. 구명칭 `in_ready=Y`는 최종 READY가 아닙니다.

| 구분 | 의미 | 수치 검증 여부 |
|---|---|---|
| `verification_input_ready=Y` | KOSIS로 검증해볼 만한 measurement 입력 | 아직 수치 검증하지 않음 |
| `mapping_status=READY` | 공식 메타와 실제 API exact match를 통과한 확정 매핑 | 수치 검증 가능 |

즉 `verification_input_ready=Y` 전체를 바로 수치 검증하지 않고, 잘못된 ITEM/OBJ 조합으로 인한 오판정을 막기 위해 `mapping_status=READY`인 후보만 실제값 비교에 사용합니다.

#### Mapping-end 검증 레이어 실험 결과

`hcx_v15_monthfix` 실험에서 후보 row 144개를 공식 메타/API exact match 기준으로 검증했습니다.
이 단계는 ITEM/OBJ를 의미적으로 완전 자동 확정하는 모델이 아니라, 후보 매핑이 KOSIS
공식 메타데이터에 존재하고 실제 API 응답과 exact match되는지 확인하는 **보수적 검증
레이어**입니다. 따라서 API exact match는 기술적 유효성 검증이며, 뉴스 의미와의 최종
일치성은 검색 점수, 단위·기간 검증, HITL 보정, 독립 holdout 평가로 계속 확인해야 합니다.

| mapping_status | 개수 | 의미 |
|---|---:|---|
| `NOT_EVALUATED` | 84 | 후순위 후보라 API 검증 생략 |
| `NEEDS_CONFIRMATION` | 32 | 자동 확정하기 어렵고 사람 확인 필요 |
| `API_ERROR` | 18 | API 조회 실패 또는 데이터 없음 |
| `READY` | 10 | 공식 메타/API exact match 통과 후 자동 확정 |

READY로 확정된 10개 후보의 실제 수치 검증 결과는 다음과 같습니다.
...
골드셋에서 `gold_itm_id`와 `gold_obj_l1`이 모두 존재하는 20개 measurement 중, 파이프라인이 자동 확정한 READY 후보는 7개였습니다. READY 후보 기준 정교도는 다음과 같습니다.

| 지표 | 값 |
|---|---:|
| READY coverage | 7/20 = 35.0% |
| READY ITEM accuracy | 7/7 = 100.0% |
| READY OBJ accuracy | 6/7 = 85.7% |
| READY ITEM/OBJ 동시 정확도 | 6/7 = 85.7% |
| 전체 gold 기준 end-to-end ITEM/OBJ 성공률 | 6/20 = 30.0% |

따라서 현재 구조는 coverage를 무리하게 늘리기보다, 확실하지 않은 후보를
`NEEDS_CONFIRMATION`으로 남기고 READY 확정 결과의 정합성을 높이는
**precision-oriented validation**입니다. 향후 과제는 이 READY 정교도를 유지하면서
HITL 보정값과 코드셋을 축적해 READY coverage를 높이는 것입니다.

안전 병합을 위해 다음 기본 정책을 적용했습니다.

- 기존 upstream `selected_itm_id`/`selected_obj`는 힌트로만 사용하고, 강제 semantic score를 주지 않습니다.
- 후보 조합 API 검증의 기본 한도는 `--api-sample-limit 0`(무제한)으로 두어 기본 실행 재현성을 확보합니다.
- Chroma-ready 기준 검색 기본값은 `hybrid`이며, 오프라인/재현성 비교가 필요할 때만 `lexical`을 명시합니다.
- 단위 family를 보강해 `개사/업체`, `달러/천달러/백만달러`, `대/건/가구` 등을 더 보수적으로 비교합니다.
- 골드 `gold_obj_l1`에 축 ID와 코드가 함께 들어간 경우 채점 시 코드 부분만 비교합니다.
- READY precision은 개발 골드 기준 수치이므로, 최종 성능 주장은 독립 holdout으로 재평가해야 합니다.

API 오류도 단순 실패로 묶지 않고 원인을 세분화했습니다.

| API error type | 개수 | 의미 |
|---|---:|---|
...
테스트 개수는 브랜치와 설치된 의존성에 따라 달라질 수 있으므로, 병합 전 현재 환경에서
다시 실행해 확인합니다.
