...
def infer_period_from_context(row: dict) -> tuple[str, str, str]:
    ...
    # Do not infer the article year from vague expressions such as 올해/금년/올해들어.
    # In service, this can inflate READY while querying the wrong KOSIS period.
    # Only explicit periods (e.g. 2025년, 2025년 1월, 올해 1월) are normalized above.
    return "", "", ""
...
def exclusion(row: dict, dimension: str, semantic: str):
    ...
    if is_policy_or_condition_text(row):
        return "NOT_KOSIS_VALUE", "정책·조건·전망·예상값은 KOSIS 실제 관측값 검증 대상이 아님"
