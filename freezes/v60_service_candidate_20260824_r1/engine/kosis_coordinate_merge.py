...
def validate_suggestion_packet(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # A safe coordinate gate may legitimately leave fewer than the nominal
    # Top-K.  Accept a contiguous rank prefix (including an empty abstention)
    # rather than dropping the whole claim or padding it with guesses.
    if len(suggestions) > len(expected_ranks) or actual_ranks != expected_ranks[:len(suggestions)]:
        raise CoordinateContractError(
            f"{source} suggestions must contain a rank prefix of "
            f"{list(expected_ranks)} in order"
        )
