...
- measurement_indicator는 지표, measurement_item은 품목·산업·대상으로 분리한다. 예: 반도체수출액 1419억 달러는 indicator=수출액, item=반도체다.
- 한 문장에 여러 품목이 있으면 각 수치를 가까운 품목과 연결한다. 바이오헬스·농수산식품·화장품 값을 하나의 결합 indicator로 묶지 않는다.
- 현재값과 이전값의 연도가 다르면 measurement_period도 각각 다르게 쓴다. 기사 제목과 이전 두 문장에 명시된 연도를 활용한다.
- 수주·도입·발표·조사 시작 연도처럼 원인이나 배경을 설명하는 연도는 관측값의 measurement_period로 쓰지 않는다. 예: '2021년 수주한 선박이 2024년에 256억 달러 수출'의 수출액 시점(measurement_period)은 2024다.
- 기사 날짜는 '지난달', '지난 8월', '올해' 같은 상대 시점을 계산하는 기준일로만 사용한다. 기사 날짜 자체를 통계 관측 시점으로 복사하지 않는다.
- 연도가 생략된 N월은 기사 날짜보다 미래가 되지 않는 가장 최근 N월로 해석한다. 예: 기사 날짜가 2026-02-25이면 '지난 8월'은 202508이며 202608이 아니다.
- '지난달'은 기사 날짜에서 한 달 전, '지난해 N월'과 '작년 N월'은 기사 연도의 직전 연도 N월로 계산한다.
- 제목·현재 문장·이전 두 문장에서 명시 또는 상대 시점으로 확정할 수 없으면 measurement_period와 measurement_prd_se를 -로 두고 추측하지 않는다.
...
OUT_COLS = ["claim_id", "claim_measurement_id", "article_id", "title", "date", "url",
            "claim_text", "prev_sentence", "prev_prev_sentence", "next_sentence",
...
PROMPT_VERSION = "v1.7-relative-period"
RETRIEVAL_PROMPT_VERSION = "v1.8-relative-period-early-kosis-context"
...
def get_prev_prev_sentence(claim):
    """Return the second preceding sentence, accepting common input aliases."""
    for field in ("prev_prev_sentence", "pre_prev_sentence", "전전_문장", "이전이전_문장"):
...
MONTH_MENTION_RE = re.compile(r"(?<!\d)(1[0-2]|[1-9])\s*월")


def _article_date(claim):
    match = re.search(
    ...
    return " ".join(
        [
            *[
            ...
            norm(get_prev_prev_sentence(claim)),
            article_context,
...
def relative_month_periods(claim):
    """기사 날짜를 기준으로 문맥에 근거한 상대 월 후보를 반환한다."""
    article_date = _article_date(claim)
    if article_date is None:
        return set()

    evidence = _period_evidence(claim)
    periods = set()
    if re.search(r"지난\s*달", evidence):
        year, month = _shift_month(article_date.year, article_date.month, -1)
        periods.add(f"{year:04d}{month:02d}")
    if re.search(r"(?:이달|이번\s*달|당월)", evidence):
        periods.add(f"{article_date.year:04d}{article_date.month:02d}")

    for match in MONTH_MENTION_RE.finditer(evidence):
        prefix = evidence[max(0, match.start() - 14):match.start()]
        if re.search(r"(?:19|20)\d{2}\s*년\s*$", prefix):
            continue
        month = int(match.group(1))
        if re.search(r"재작년\s*$", prefix):
            year = article_date.year - 2
        elif re.search(r"(?:지난해|작년|전년)\s*$", prefix):
            year = article_date.year - 1
        elif re.search(r"(?:내년|다음\s*해)\s*$", prefix):
            year = article_date.year + 1
        elif re.search(r"(?:올해|금년|이번\s*해)\s*$", prefix):
...
def infer_relative_month_period(claim):
    """문맥의 상대 월이 하나로 확정될 때 YYYYMM을 반환한다."""
    periods = relative_month_periods(claim)
    return next(iter(periods)) if len(periods) == 1 else "-"
...
def period_is_grounded(period, claim):
    ...
    if value in relative_month_periods(claim):
        return True
...
def apply_relative_month_periods(result, claim):
    """HCX의 미래 월 오해를 기사 날짜 이전의 최근 상대 월로 교정한다."""
    resolved = infer_relative_month_period(claim)
    if resolved == "-":
        return 0

    corrected = 0
    original_claim_period = norm(result.get("period"))
    if original_claim_period != resolved:
        result["period"] = resolved
        result["prd_se"] = "M"
        result["time_resolution_status"] = "확정"
        corrected += 1

    article_date = _article_date(claim)
    article_period = (
        f"{article_date.year:04d}{article_date.month:02d}" if article_date else ""
    )
    for measurement in result.get("measurements") or []:
        period = norm(measurement.get("measurement_period"))
        should_correct = period == original_claim_period and period != resolved
        if re.fullmatch(r"(?:19|20)\d{4}", period):
            should_correct = should_correct or (
                period[4:] == resolved[4:] and article_period and period > article_period
            )
        if should_correct:
...
def remove_ungrounded_claim_period(result, claim):
    """근거 없는 claim-level 기간을 fallback 전에 제거한다."""
    period = norm(result.get("period"))
    if period == "-" or period_is_grounded(period, claim):
        return 0
    result["period"] = "-"
    result["prd_se"] = "-"
    if norm(result.get("time_resolution_status")) == "확정":
        result["time_resolution_status"] = "모호"
    return 1
...
def ensure_measurement_bindings(result):
    ...
    A measurement without its own period inherits the claim-level period and
    periodicity when available.  If the claim-level values are also missing,
    the measurement values remain unresolved.
    """
    fallback_values = {
...
def extract_claim(api_key, model, claim, effort="none"):
    ...
    common = {
        ...
        "prev_prev": get_prev_prev_sentence(claim),
        "candidates": candidates,
        "effort": effort,
    ...
    relative_period_corrected_count = apply_relative_month_periods(result, claim)
    claim_period_removed_count = remove_ungrounded_claim_period(result, claim)
    ...
    if (fallback_count or binding_fallback_count or period_removed_count
            or claim_period_removed_count or relative_period_corrected_count):
        ...
        if claim_period_removed_count:
            fallback_reasons.append(f"claim_period_ungrounded:{claim_period_removed_count}")
        if relative_period_corrected_count:
            fallback_reasons.append(
                f"relative_month_period_corrected:{relative_period_corrected_count}"
            )
    ...
    result["_claim_period_removed_count"] = str(claim_period_removed_count)
    result["_relative_period_corrected_count"] = str(relative_period_corrected_count)
...
def to_rows(claim, j, model):
    ...
    base["prev_prev_sentence"] = norm(get_prev_prev_sentence(claim))
    base.update({
