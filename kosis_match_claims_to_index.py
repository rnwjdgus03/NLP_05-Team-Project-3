...
DEFAULT_PERIOD_ROUTE_INDEX = PROJECT_DIR / "data/claims/kosis_period_table_routes.csv"
...
def normalized_claim_row(row):
    ...
    return {
        ...
        "direction": get_first(row, "direction"),
...
def load_period_routes(path: Path):
    """Load period-aware table routing rules.

    KOSIS sometimes splits one real-world series across several table IDs or
    table names by year.  These rules do not verify a value by themselves; they
    only inject the period-compatible table into the retrieval candidate list so
    downstream official metadata + KOSIS API exact validation can decide.
    """
    ...
    routes = []
    for row in rows:
        if str(row.get("enabled", "Y")).strip().upper() in {"N", "NO", "FALSE", "0"}:
            continue
        routes.append({
            "series_group": row.get("series_group", ""),
            "indicator_regex": row.get("indicator_regex", ""),
            "item_regex": row.get("item_regex", ""),
            "period_start": row.get("period_start", ""),
            "period_end": row.get("period_end", ""),
            "org_id": row.get("org_id", ""),
            "tbl_id": row.get("tbl_id", ""),
            "tbl_name_contains": row.get("tbl_name_contains", ""),
            "route_status": row.get("route_status", "PROVISIONAL_PERIOD_ROUTED"),
...
def claim_year(claim):
    ...
    for value in (claim.get("period"), claim.get("comparison_period")):
...
def _route_regex_match(pattern, text):
...
def matching_period_routes(claim, routes):
    if not routes:
        return []
    norm_claim = normalized_claim_row(claim)
    year = claim_year(norm_claim)
    if year is None:
        return []
    indicator_text = f"{norm_claim.get('indicator', '')} {norm_claim.get('claim_text', '')}"
    item_text = f"{norm_claim.get('industry_or_item', '')} {norm_claim.get('claim_text', '')}"
    matched = []
    for route in routes:
        try:
            start = int(str(route.get("period_start") or "0"))
            end = int(str(route.get("period_end") or "9999"))
        except ValueError:
            continue
        if not (start <= year <= end):
            continue
        if not _route_regex_match(route.get("indicator_regex"), indicator_text):
            continue
        if not _route_regex_match(route.get("item_regex"), item_text):
            continue
        matched.append(route)
    return matched


def period_route_candidates(table_rows, claim, routes):
    matched_routes = matching_period_routes(claim, routes)
    if not matched_routes:
        return []
    out = []
    for route in matched_routes:
        name_hint = compact(route.get("tbl_name_contains"))
        route_org = str(route.get("org_id") or "")
        route_tbl = str(route.get("tbl_id") or "")
        for table in table_rows:
            if route_org and table.get("org_id") != route_org:
                continue
            if route_tbl and table.get("tbl_id") != route_tbl:
                continue
            if name_hint and name_hint not in table.get("_compact_tbl_name", ""):
                continue
            if not route_tbl and not name_hint:
                continue
            out.append({
                "score": 50000,
                "hits": ["period_route", route.get("series_group", "")],
                "table": table,
                "retrieval_backend": "period_route+lexical",
                "lexical_score": 50000,
                "lexical_eligible": True,
                "semantic_score": None,
                "reranker_score": None,
                "fusion_score": None,
                "period_route_series_group": route.get("series_group", ""),
                "period_route_status": route.get("route_status", "PROVISIONAL_PERIOD_ROUTED"),
                "period_route_basis": route.get("mapping_basis", ""),
                "period_route_definition_note": route.get("definition_note", ""),
            })
    # If tbl_id and name both match multiple duplicates, preserve one per table.
    deduped = {}
    for item in out:
...
def merge_period_routes(ranked, routed, top_tables):
    if not routed:
        return ranked[:top_tables]
    by_key = {table_key(item["table"]): item for item in routed}
    merged = []
    for item in ranked:
        key = table_key(item["table"])
        if key in by_key:
            route_item = by_key.pop(key)
            route_item["score"] = max(route_item["score"], item.get("score", 0) + 10000)
            route_item["hits"] = list(dict.fromkeys(route_item["hits"] + item.get("hits", [])))
            route_item["semantic_score"] = item.get("semantic_score")
            route_item["reranker_score"] = item.get("reranker_score")
            route_item["fusion_score"] = item.get("fusion_score")
            merged.append(route_item)
        else:
            merged.append(item)
    merged = list(by_key.values()) + merged
    merged.sort(
        key=lambda item: (
            0 if str(item.get("period_route_status", "")).startswith("CONFIRMED") else 1,
            -item.get("score", 0),
            item["table"]["tbl_name"],
        )
    )
    return merged[:top_tables]
...
def rank_table_candidates(
    ...
    routed = period_route_candidates(table_rows, norm_claim, period_routes or [])
    lexical = []
    ...
    if semantic_runtime is None:
        ...
        return merge_period_routes(ranked, routed, top_tables)
    ...
    return merge_period_routes(fused, routed, top_tables)
        ...
        grouped[measurement_key].append(
            {
                ...
                "period_route_series_group": row.get("period_route_series_group", ""),
                "period_route_status": row.get("period_route_status", ""),
                "period_route_basis": row.get("period_route_basis", ""),
                "period_route_definition_note": row.get("period_route_definition_note", ""),
            }
        )
...
def main():
    ...
    period_routes = load_period_routes(Path(args.period_route_index).expanduser())
    ...
    for claim in claims:
        ...
        for rank, candidate in enumerate(ranked, 1):
            ...
            out.append({
                ...
                "direction": norm_claim.get("direction", ""),
                ...
                "period_route_series_group": candidate.get("period_route_series_group", ""),
                "period_route_status": candidate.get("period_route_status", ""),
                "period_route_basis": candidate.get("period_route_basis", ""),
                "period_route_definition_note": candidate.get("period_route_definition_note", ""),
                "org_id": table["org_id"],
                "tbl_id": table["tbl_id"],
    ...
    fields = [
        ...
        "period_route_series_group", "period_route_status", "period_route_basis", "period_route_definition_note",
    ...
    print(
        ...
        f"meta_tables={len(meta_by_table)} period_routes={len(period_routes)} retrieval={retrieval_mode}"
    )
    if excluded:
