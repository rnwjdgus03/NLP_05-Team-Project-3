...
ARTIFACTS = {
    ...
    "contexts": "03_claim_contexts.csv",
...
def evaluate_run(run_dir: Path) -> dict[str, Any]:
    ...
    counts = {
        ...
        "claim_contexts": len(rows["contexts"]),
...
def metric_rows(result: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    rows = [
        ...
        (
            "context",
            "claim_contexts",
            counts["claim_contexts"],
            counts["claim_spans"],
            percentage(counts["claim_contexts"], counts["claim_spans"]),
            "claim spans with rebuilt article context",
        ),
