...
def _positive_scope_hints(claim):
    ...
    # KOSIS publishes household balance-sheet values under the
    # 가계금융복지조사 family. News wording usually describes the household and
    # asset directly instead of naming that survey. Add the official taxonomy
    # view only when the bound measurement indicator contains both concepts;
    # an article title about generic real-estate prices is not enough.
    if "가구" in measurement_indicator and "자산" in measurement_indicator:
        hints.append("가계금융복지조사 입주형태별 자산 부채 소득 현황")
    return tuple(hints)
