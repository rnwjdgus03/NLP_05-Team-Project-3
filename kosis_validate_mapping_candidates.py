...
def main() -> None:
    ...
    parser.add_argument("--max-combinations", type=int, default=5, help="Default API-candidate Top-K. Use 20 only for debug/recall mode")
    parser.add_argument("--api-candidate-top-k", type=int, default=None, help="Alias for --max-combinations; default 5")
    ...
    parser.add_argument("--chroma-coordinate-index", default="",
                        help="Persistent ChromaDB directory for coordinate-level ITEM/OBJ retrieval")
    parser.add_argument("--chroma-collection-name", default="kosis_coordinate_bge_m3_v1")
    ...
    parser.add_argument("--chroma-validation-top-k", type=int, default=20, help="Coordinate search Top-K before API candidate pruning")
    parser.add_argument("--no-chroma-reranker", action="store_true")
    parser.add_argument("--chroma-max-documents", type=int, default=0, help="Limit fallback coordinate documents; 0 means all")
    args = parser.parse_args()
    if args.api_candidate_top_k is not None:
        args.max_combinations = args.api_candidate_top_k
    ...
    all_meta_rows: list[dict[str, str]] = []
    ...
    if args.chroma_coordinate_index:
        coordinate_runtime = CoordinateSearchRuntime(
            Path(args.chroma_coordinate_index),
    ...
    for row in work:
        ...
        if coordinate_runtime is not None:
            ...
            for order, candidates in chroma_objs.items():
                obj_candidates[order] = _merge_seeded_candidates(
                    obj_candidates.get(order, []),
                    candidates,
                )
        ...
        outputs.append({**row, **result, "mapping_status_schema_version": "v2_ready_is_exact_api_match"})
