...
| 확인 불가 | 31 (30%) | 기간이 기사일 이후 15 · 좌표는 맞고 데이터 없음 11 · 기간 불명 5 |
| 시스템 한계 | 60 (58%) | 후보 비결정 27 · 단위 확인 불가 21 · 좌표 오류 12 |

- **전체 기준 11.7%** — 사용자가 기사를 넣었을 때 실제로 답을 받을 확률
- **확인 가능 기준 16.7%** — 답할 수 있는 문제에서 시스템이 얼마나 하는가
...
| 상류 추출이 문장에 없는 대상을 부착 | 2 | `UNGROUNDED_CLAIM_ITEM` — 문장에 없는 대상은 확정 보류 |
| 한계값 서술을 값 주장으로 처리 | 1 | `THRESHOLD_CLAIM_UNSUPPORTED` — 판정하지 않음 |
