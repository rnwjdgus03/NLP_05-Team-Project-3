...
def recover_explicit_claim_period(row: dict) -> tuple[str, str]:
    """Inherit a complete claim month only when the same sentence grounds it.

    HCX sometimes binds the period at claim level but leaves every measurement
    period as ``-``. Blindly copying it is unsafe for historical side facts and
    cumulative ranges, so this repair requires one explicit calendar month in
    the same measurement sentence.
    """
    original = measurement_value(row, "measurement_period", "period")
    if nz(original) not in {"", "-"}:
        return original, ""
    periodicity = claim_value(row, "claim_prd_se", "prd_se").upper()
    if periodicity != "M":
        return original, ""
    claim_period = canonicalize_period(
        claim_value(row, "claim_period", "period"), periodicity
    )
    if not re.fullmatch(r"(?:19|20)\d{4}", claim_period):
        return original, ""
    text = nz(row.get("claim_text"))
    month = int(claim_period[-2:])
    months = {int(match.group(1)) for match in _CALENDAR_MONTH.finditer(text)}
    if months != {month}:
        return original, ""
    # ``1~8월`` and ``1월부터 8월까지`` are cumulative spans, not August.
    if (
        re.search(
            rf"\d{{1,2}}\s*(?:~|∼|～|−|–|—|-)\s*0?{month}\s*월", text
        )
        or re.search(
            rf"\d{{1,2}}\s*월\s*(?:부터|~|∼|～|−|–|—|-)\s*0?{month}\s*월",
            text,
        )
        or "누적" in text
    ):
    ...
    explicit_years = {
        int(value)
        for value in re.findall(r"(?<!\d)((?:19|20)\d{2})\s*년", text)
    }
    if explicit_years and explicit_years != {int(claim_period[:4])}:
        return original, ""
    return claim_period, "EXPLICIT_CLAIM_MONTH_INHERITED"
...
def normalize_row(row: dict) -> dict:
    ...
    inherited_period, inherited_status = recover_explicit_claim_period(repaired_row)
    inherited_row = dict(repaired_row)
    if "measurement_period" in repaired_row:
        inherited_row["measurement_period"] = inherited_period
    else:
        inherited_row["period"] = inherited_period
    relative_period, relative_status = recover_relative_measurement_period(inherited_row)
    ...
    if inherited_status or contextual_status:
    ...
    if (inherited_status or contextual_status) and not out["prd_se"]:
