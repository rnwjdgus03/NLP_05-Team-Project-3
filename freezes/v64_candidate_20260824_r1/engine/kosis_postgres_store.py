...
def aggregate_component_recall_matches(
    rows: Sequence[Mapping[str, Any]], *, limit: int,
) -> list[dict[str, Any]]:
    """Collapse exact component hits while preserving cross-term evidence.

    The SQL returns one row per (query term, table).  Previously the first row
    for a table won and later evidence was discarded.  Accumulating all terms
    lets a table that supports both an indicator ITEM and a requested OBJ beat
    a table that merely matches one broad word.
    """
    ...
    for row in rows:
        ...
        entry = by_table.setdefault(key, {
            ...
            "score": 0.0, "first_term_order": 10**9, "term_rank": 10**9,
            "matched_item_names": set(), "matched_component_sources": set(),
            "matched_term_orders": set(),
        })
        input_order = int(row.get("input_order") or 10**9)
        entry["score"] = max(float(entry["score"]), float(row.get("match_quality") or 0.0))
        entry["first_term_order"] = min(int(entry["first_term_order"]), input_order)
        entry["term_rank"] = min(int(entry["term_rank"]), int(row.get("term_rank") or 10**9))
        entry["matched_term_orders"].add(input_order)
        entry["matched_item_names"].update(
            _text(value) for value in (row.get("matched_item_names") or []) if _text(value)
        )
        entry["matched_component_sources"].update(
            _text(value) for value in (row.get("matched_component_sources") or []) if _text(value)
        )
    ranked = sorted(by_table.values(), key=lambda row: (
        -len(row["matched_term_orders"]),
        -len(row["matched_component_sources"]),
        int(row["first_term_order"]), int(row["term_rank"]),
        row["org_id"], row["tbl_id"],
    ))
    output = []
    for rank, row in enumerate(ranked[:max(0, int(limit))], 1):
        output.append({
            **row,
            "rank": rank,
            "matched_term_count": len(row["matched_term_orders"]),
            "matched_term_orders": sorted(row["matched_term_orders"]),
            "matched_item_names": sorted(row["matched_item_names"]),
            "matched_component_sources": sorted(row["matched_component_sources"]),
        })
    return output
...
class PostgresKosisMetadataStore:
    ...
    def item_table_recall(
        self,
        terms: Sequence[str],
        *,
        limit: int = 50,
        prd_se: str = "",
        balanced_terms: bool = True,
    ) -> list[dict[str, Any]]:
        """Find table candidates through official ITEM and OBJ names.

        This is a bounded table-level recall channel, not an ITEM/OBJ vector
        index. Exact and prefix ITEM-name matches are collapsed to distinct
        tables before Stage A reranking.
        """
        cleaned = []
        for value in terms:
            term = _text(value)
            if len(re.sub(r"[^0-9A-Za-z가-힣]", "", term)) < 2:
                continue
        ...
        if self._metadata_guard_depth == 0:
            with self.metadata_read_guard():
                return self.item_table_recall(
                    cleaned, limit=limit, prd_se=prd_se,
                    balanced_terms=balanced_terms,
                )
        snapshot = self._guard_snapshot or self.active_snapshot()
        if snapshot is None:
            raise RuntimeError("no active KOSIS metadata snapshot")
        values_sql = ",".join(["(%s,%s)"] * len(cleaned))
        parameters: list[Any] = []
        for position, term in enumerate(cleaned, 1):
            parameters.extend((position, term))
        periodicity = _text(prd_se).upper()
        sql = f"""
            WITH terms(input_order, term) AS (VALUES {values_sql}),
            raw_matches AS (
                SELECT terms.input_order, terms.term, matched.org_id, matched.tbl_id,
                       matched.component_name, 'ITEM'::TEXT AS component_source,
                       3 AS match_quality
                FROM terms
                CROSS JOIN LATERAL (
                    SELECT item.org_id, item.tbl_id,
                           item.item_name AS component_name
                    FROM kosis_items item
                    WHERE lower(item.item_name)=lower(terms.term)
                    ORDER BY item.org_id, item.tbl_id
                    LIMIT %s
                ) matched
                UNION ALL
                SELECT terms.input_order, terms.term, matched.org_id, matched.tbl_id,
                       matched.component_name, 'OBJ'::TEXT AS component_source,
                       3 AS match_quality
                FROM terms
                CROSS JOIN LATERAL (
                    SELECT value.org_id, value.tbl_id,
                           value.value_name AS component_name
                    FROM kosis_axis_values value
                    WHERE lower(value.value_name)=lower(terms.term)
                    ORDER BY value.org_id, value.tbl_id
                    LIMIT %s
                ) matched
            ), matched AS (
                SELECT component.input_order, component.org_id, component.tbl_id,
                       MAX(component.match_quality) AS match_quality,
                       array_agg(DISTINCT component.component_name ORDER BY component.component_name)
                           AS matched_item_names,
                       array_agg(DISTINCT component.component_source ORDER BY component.component_source)
                           AS matched_component_sources
                FROM raw_matches component
                WHERE (%s='' OR EXISTS (
                    SELECT 1 FROM kosis_periodicities period
                    WHERE period.org_id=component.org_id
                      AND period.tbl_id=component.tbl_id
                      AND upper(period.prd_se)=%s
                ))
                GROUP BY component.input_order, component.org_id, component.tbl_id
            ), ranked AS (
                SELECT matched.*,
                       row_number() OVER (
                           PARTITION BY matched.input_order
                           ORDER BY matched.match_quality DESC,
                                    matched.org_id, matched.tbl_id
                       ) AS term_rank
                FROM matched
            )
            SELECT ranked.input_order, ranked.org_id, ranked.tbl_id,
                   ranked.term_rank, ranked.match_quality,
                   ranked.matched_item_names, ranked.matched_component_sources
            FROM ranked
            JOIN kosis_snapshot_tables table_meta
              ON table_meta.snapshot_id=%s
             AND table_meta.org_id=ranked.org_id
             AND table_meta.tbl_id=ranked.tbl_id
            WHERE ranked.term_rank <= %s
            -- Terms are ordered by the structured metric heuristic in Stage A.
            -- Give that authoritative first metric a quota before expanding
            -- to noisier context tokens; otherwise dozens of terms can push a
            -- valid ITEM match beyond the bounded reranker pool.
            ORDER BY ranked.input_order, ranked.term_rank,
                     ranked.match_quality DESC, ranked.org_id, ranked.tbl_id
            LIMIT %s
        """
        component_cap = max(200, int(limit) * 4)
        term_quota = (
            max(10, math.ceil(int(limit) / len(cleaned)))
            if balanced_terms else int(limit)
        )
        parameters.extend((
            component_cap, component_cap,
            periodicity, periodicity, _text(snapshot["snapshot_id"]),
            term_quota, int(limit) * 4,
        ))
        with self.connection.cursor() as cursor:
        ...
        for row in rows:
            if isinstance(row, Mapping):
                ...
                normalized_rows.append({
                    "input_order": values[0], "org_id": values[1],
                    "tbl_id": values[2], "term_rank": values[3],
                    "match_quality": values[4],
                    "matched_item_names": values[5],
                    "matched_component_sources": values[6],
                })
        return aggregate_component_recall_matches(normalized_rows, limit=int(limit))

    def table_metadata_recall(
        self,
        terms: Sequence[str],
        *,
        limit: int = 200,
        ...
        """Recall tables through official table/survey/category text.

        This is a bounded PostgreSQL FTS channel complementary to BGE and
        ITEM/OBJ recall.  It returns table identifiers only; the existing
        period, scope, metadata, and cross-encoder guards remain authoritative.
        """
        cleaned: list[str] = []
        for value in terms:
            term = _text(value)
        ...
        if self._metadata_guard_depth == 0:
            with self.metadata_read_guard():
                return self.table_metadata_recall(
                    cleaned, limit=limit, prd_se=prd_se,
                )
        snapshot = self._guard_snapshot or self.active_snapshot()
        if snapshot is None:
        ...
        sql = f"""
            WITH terms(input_order, term) AS (VALUES {values_sql}),
            matched AS (
                SELECT table_meta.org_id, table_meta.tbl_id,
                       MIN(terms.input_order) AS first_term_order,
                       COUNT(DISTINCT terms.input_order) AS matched_term_count,
                       MAX(ts_rank_cd(
                           to_tsvector('simple', table_meta.search_document),
                           plainto_tsquery('simple', terms.term)
                       )) AS text_rank
                FROM terms
                JOIN kosis_snapshot_tables table_meta
                  ON table_meta.snapshot_id=%s
                 AND to_tsvector('simple', table_meta.search_document)
                     @@ plainto_tsquery('simple', terms.term)
                WHERE (%s='' OR EXISTS (
                    SELECT 1 FROM kosis_periodicities period
                    WHERE period.org_id=table_meta.org_id
                      AND period.tbl_id=table_meta.tbl_id
                      AND upper(period.prd_se)=%s
                ))
                GROUP BY table_meta.org_id, table_meta.tbl_id
            )
            SELECT org_id,tbl_id,first_term_order,matched_term_count,text_rank
            FROM matched
            ORDER BY matched_term_count DESC,first_term_order,text_rank DESC,
                     org_id,tbl_id
            LIMIT %s
        """
        parameters.extend((
            _text(snapshot["snapshot_id"]), periodicity, periodicity, int(limit),
        ))
        with self.connection.cursor() as cursor:
            cursor.execute(sql, tuple(parameters))
            rows = cursor.fetchall()
        ...
        for rank, row in enumerate(rows, 1):
            if isinstance(row, Mapping):
                ...
                values = {
                    "org_id": row[0], "tbl_id": row[1],
                    "first_term_order": row[2], "matched_term_count": row[3],
                    "text_rank": row[4],
                }
            normalized.append({
                "org_id": _text(values.get("org_id")),
                "tbl_id": _text(values.get("tbl_id")),
                "rank": rank,
                "first_term_order": int(values.get("first_term_order") or 10**9),
                "matched_term_count": int(values.get("matched_term_count") or 1),
                "score": float(values.get("text_rank") or 0.0),
            })
        return normalized
        ...
        sql = f"""
            ...
            SELECT requested.input_order,
                   ...
                   COALESCE((
                       ...
                       WHERE period.org_id=requested.org_id
                         AND period.tbl_id=requested.tbl_id
                   ), ARRAY[]::TEXT[]) AS periodicities,
                   COALESCE((
                       SELECT jsonb_agg(jsonb_build_object(
                           'prd_se', period.prd_se,
                           'range_text', period.range_text
                       ) ORDER BY period.prd_se)
                       FROM kosis_periodicities period
                       WHERE period.org_id=requested.org_id
                         AND period.tbl_id=requested.tbl_id
                   ), '[]'::jsonb) AS period_ranges,
        ...
        for row in rows:
            if isinstance(row, Mapping):
                ...
                values = (
                    ...
                    get("unit_hints"),
                    get("periodicities"), get("period_ranges"),
                    get("catalog_present"),
            ...
            output.append({
                ...
                "period_ranges": [
                    {
                        "prd_se": _text(value.get("prd_se")).upper(),
                        "range_text": _text(value.get("range_text")),
                    }
                    for value in (values[8] or []) if isinstance(value, Mapping)
                ],
                "catalog_present": bool(values[9]),
                "metadata_complete": bool(values[10]),
            })
        return output
    ...
    def hydrate_component_bundle(
        ...
        for row in rows:
            ...
            table = {
                "rank": table_rank,
                "stage_a_channel": _text(source.get("stage_a_channel")),
                "stage_a_channel_rank": source.get("stage_a_channel_rank"),
                "org_id": org_id,
                "tbl_id": tbl_id,
            ...
            common = {
                ...
                "prd_se": table["prd_se"],
                "stage_a_channel": table["stage_a_channel"],
                "stage_a_channel_rank": table["stage_a_channel_rank"],
            }
