...
def build_coordinate_documents(meta_rows: Iterable[Mapping[str, Any]], *, max_documents: int = 0) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if row["obj_id"].upper() == "ITEM" and row["itm_id"]:
            table["items"][row["itm_id"]] = row["itm_name"]
    ...
    for table in by_table.values():
        ...
        if not table["items"] or not axis_values:
            continue
        for itm_id, itm_name in sorted(table["items"].items()):
            paths = [[]]
            ...
            for path in paths:
                ...
                text_parts = [
                    f"통계표: {table['tbl_name']}",
                    f"분류 경로: {table['category_path']}",
                    f"항목: {itm_name}",
                ]
                ...
                for value in path:
                    ...
                    metadata[f"obj_l{level}"] = value["itm_id"]
                    metadata[f"obj_l{level}_name"] = value["itm_name"]
                    obj_names.append(value["itm_name"])
                    id_parts.extend([level, value["obj_id"], value["itm_id"]])
                    text_parts.append(f"분류{level}: {value['obj_name']}={value['itm_name']}")
                metadata["obj_path"] = " > ".join(obj_names)
