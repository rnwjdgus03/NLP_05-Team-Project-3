...
현재 `kosis_table_summary.csv`에는 30개 최상위 카테고리 중 25개(농림·인구·노동·물가·사회일반·소득소비자산·경제일반경기·무역국제수지·임금·도소매서비스·범죄안전·보건·복지·교육훈련·문화여가·주거·국토이용·기업경영·수산·광업제조업·건설·교통물류·정보통신·과학기술·국민계정)가 크롤링되어, 총 86,060개 통계표가 인덱싱되어 있습니다. 남은 5개(정부재정·금융·환경·에너지·지역통계)는 진행 중입니다.
...
- B팀: KOSIS API 연동, 통계표 인덱스(86,060개, 30개 카테고리 중 25개 완료), 메타정보 조회, 파라미터 가이드, 자동 매칭 스크립트까지 준비 완료
...
- 2026.07.13 추가: 범죄·안전(C), 보건(F), 복지(G), 교육·훈련(H1), 문화·여가(H2), 주거(I1), 국토이용(I2), 기업경영(J2), 수산(K2), 광업·제조업(L), 건설(M1), 교통·물류(M2), 정보통신(N1), 과학·기술(N2), 국민계정(Q) 카테고리 크롤링 완료 (총 86,060건). 남은 5개(정부·재정(R), 금융(S1), 환경(T), 에너지(U), 지역통계(V))는 지역통계(V)의 트리 규모가 유난히 커서 시간이 오래 걸려 다음 세션에 이어서 진행 예정
