...
- **verify_claim.py**: 골격 완성, `CHANGE_RATE`/`LEVEL`/`ABS_TO_ABS` 판정 로직 구현됨. `obj_l1`/`itm_id`/`prd_se`가 채워진 매핑에 대해 실제 KOSIS 값 조회 후 일치/불일치 판정 가능.
  ...
  - 2,001건 전체 최신 상태는 확정 일치 21건/재검토 1,643건/판단불가 337건이다. 상세 근거는 `outputs/bteam_review/submission_recheck_8_report.md`에 기록했다.
...
- **다음 할 일**:
  1. `C20191` 과배제를 먼저 수정해 정책 배경 단어보다 명시적인 KOSIS 지표 매핑을 우선한다.
  2. 독립 평가에서 확인한 검증 가능 보류 26건과 검증 불가 보류 48건으로 코드북 v2를 개발한다.
  3. 최신 출생 잠정치 표 탐색과 API 갱신 지연 재시도 큐를 구현한다.
  4. 코드북 v2를 고정한 뒤 현재 홀드아웃과 겹치지 않는 새 표본에서 80% 게이트를 다시 측정한다.
  5. 새 독립 평가가 80%를 통과할 때까지 수동검토 유지 1,281건의 자동 확정 확대는 보류한다.
  6. 이후 P2 58건 후보를 선택하고 P4 4,206건의 기관·통계표를 탐색한다.
