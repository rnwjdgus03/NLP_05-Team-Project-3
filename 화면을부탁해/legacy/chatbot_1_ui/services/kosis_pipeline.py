...
def _choose_mapping(rows: list[dict[str, str]]) -> dict[str, str] | None:
    if not rows:
        return None
    ready = [row for row in rows if _text(row.get("mapping_status")) == "READY"]
...
def merge_kosis_results(
    ...
    *,
    ...
    for source in measurements:
        ...
        if verification:
            status = _text(verification.get("verdict")) or "판단불가"
            status_code = _text(verification.get("verdict_code")) or "COMPARISON_FAILED"
            status_reason = _text(verification.get("verdict_reason")) or "검증 사유 없음"
            stage = "verification"
        elif mapping:
            status = _text(mapping.get("mapping_status")) or "NEEDS_CONFIRMATION"
            status_code = status
            status_reason = _text(mapping.get("mapping_reason")) or "매핑 확정 필요"
            stage = "mapping"
        elif candidate_rows:
