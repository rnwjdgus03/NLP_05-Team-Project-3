...
입력
- run_kosis_measurement_pipeline.py가 만든 *_kosis_candidates_with_meta.csv
...
from kosis_api_test import (  # noqa: E402
    canonical_stat_request,
    get_meta,
    get_stat_data,
    safe_exception_message,
    stat_request_fingerprint,
)
...
def normalize_prd_se(prd_se, period):
    s = normalize_periodicity(prd_se)
    if s:
        return s
...
def explicit_comparison_period(row):
    """Return a comparison period explicitly grounded in the claim text.

    Explicit comparison anchors must beat change_base/default inference.
    This prevents "2022년 기록보다 2억달러 증가" from silently becoming
    period-1 (for example 2022-2021).
    """
    explicit = parse_period(row.get('comparison_period'))
    if explicit:
        return explicit, '입력 comparison_period 명시'
    ...
    for pattern in EXPLICIT_COMPARISON_PATTERNS:
        for match in re.finditer(pattern, text):
            value = parse_period(match.group(1))
            if value and value != current:
                return value, '원문 명시 비교연도 우선 적용'
    return '', ''


def infer_comparison_period(row):
    """Infer comparison period with explicit anchors before defaults."""
    explicit, reason = explicit_comparison_period(row)
    if explicit:
...
def unit_spec(unit):
    ...
    if dimension == 'currency':
        if '달러' in value:
            ...
            family = 'KRW'
        # Preserve Korean currency magnitudes.  In particular, many household
        # finance tables publish ``만원`` while news claims normalize the same
        # value to ``원``.  Longer compound magnitudes must precede their
        # substrings (``천만`` before ``만``).
        scales = [
            ('조', 1e12), ('억', 1e8), ('천만', 1e7), ('백만', 1e6),
            ('십만', 1e5), ('만', 1e4), ('천', 1e3),
        ]
    ...
    if dimension == 'rate':
        family = 'RATE_POINT' if value == '%p' else 'RATE'
        return dimension, family, 1.0
    if dimension == 'count':
...
def indicator_table_family_mismatch(row):
    """Return a reason when a named price-index family contradicts the table.

    Consumer, producer, import and export price indices can have numerically
    similar monthly changes, but they are not interchangeable evidence.  A
    vector/coordinate score must therefore never turn an explicit producer
    price claim into a consumer price MATCH (or the inverse).
    """
    claim_text = compact(' '.join(str(row.get(k, '')) for k in [
        'indicator', 'measurement_indicator', 'claim_indicator', 'claim_text',
        'measurement_text', 'keywords',
    ]))
    table_text = compact(' '.join(str(row.get(k, '')) for k in [
        'tbl_name', 'official_table_name', 'selected_itm_name',
        'official_item_name',
    ]))
    if not claim_text or not table_text:
        return ''

    families = (
        ('생산자물가', ('생산자물가',), ('소비자물가', '수입물가', '수출물가')),
        ('소비자물가', ('소비자물가',), ('생산자물가', '수입물가', '수출물가')),
        ('수입물가', ('수입물가',), ('생산자물가', '소비자물가', '수출물가')),
        ('수출물가', ('수출물가',), ('생산자물가', '소비자물가', '수입물가')),
    )
    for claim_family, accepted, conflicting in families:
        if claim_family not in claim_text:
            continue
        if any(token in table_text for token in accepted):
            return ''
        conflict = next((token for token in conflicting if token in table_text), '')
        if conflict:
            return (
                f'claim 지표 계열={claim_family}, 공식 표 계열={conflict}: '
                '서로 다른 물가지수 계열은 수치가 같아도 비교할 수 없음'
            )
    return ''
...
def needs_manual_code_review(row, obj_reason):
    ...
    KOSIS API 응답 성공은 coordinate 존재성 검증이지, 뉴스 claim 개념과 OBJ
    범위가 동일하다는 증거가 아니다. broad claim이 세부 SITC/품목 코드에
    붙으면 VALUE_MISMATCH 전에 SEMANTIC_SCOPE_REVIEW_REQUIRED로 막는다.
    """
    family_reason = indicator_table_family_mismatch(row)
    if family_reason:
        return True, family_reason

    text = compact(' '.join(str(row.get(k, '')) for k in [
        'indicator', 'measurement_indicator', 'industry_or_item', 'measurement_item',
        'keywords', 'population_etc', 'claim_text'
    ]))
    target_terms = claim_target_terms(row)
    selected_names = [
        str(row.get(f'selected_obj_l{level}_name') or '').strip()
        for level in range(1, 9)
    ]
    selected_names = [name for name in selected_names if name]
    normalized_selected = [compact(name) for name in selected_names]
    selected_objects = []
    roles = set()
    for level in range(1, 9):
        name = str(row.get(f'selected_obj_l{level}_name') or '').strip()
        axis_name = str(row.get(f'selected_obj_l{level}_axis_name') or '').strip()
        if not name and not axis_name:
            continue
        role = infer_axis_role(axis_name)
        roles.add(role)
        selected_objects.append({'axis_name': axis_name, 'name': name})
    population_requirements = claim_population_requirements(row)
    if not regional_table_scope_matches(row, row.get('tbl_name', '')):
        return True, 'claim 지역 범위와 특정 시도 통계표 범위가 일치하지 않음'
    if (
        population_requirements
        and INDUSTRY in roles
        and roles & {POPULATION_ATTRIBUTE, SEX}
    ):
        if role_aware_coordinate_scope(row, target_terms, selected_objects):
            return False, ''
        return True, '공식 산업/인구특성 축과 claim 모집단 범위가 일치하지 않음'
    if population_requirements:
        return True, '인력 claim의 공식 산업/인구특성 축 역할을 확인할 수 없음'
    if not target_scope_exact_match(target_terms, selected_names):
        return (
            True,
            f"claim target '{'|'.join(target_terms) or '<aggregate>'}'과 "
            "선택 OBJ 범위가 완전일치하지 않음: "
            + ", ".join(selected_names),
        )
    return False, ''
...
def exact_api_coordinate_rows(data, request_fields):
    """Keep only rows that echo the requested official coordinate exactly."""
    expected = {
        'ORG_ID': str(request_fields.get('orgId') or '').strip(),
        'TBL_ID': str(request_fields.get('tblId') or '').strip(),
        'ITM_ID': str(request_fields.get('itmId') or '').strip(),
        'PRD_SE': normalize_periodicity(request_fields.get('prdSe')),
    }
    for level in range(1, 9):
        value = str(request_fields.get(f'objL{level}') or '').strip()
        if value:
            expected[f'C{level}'] = value
    ...
    for row in data:
        ...
        for field, value in expected.items():
            ...
            if field == 'PRD_SE':
                actual = normalize_periodicity(actual)
...
def aggregation_method(row):
    explicit = str(row.get('period_aggregation') or '').strip().lower()
    if explicit in {'sum', 'latest'}:
        return explicit
    indicator = compact(row.get('indicator', ''))
...
_PRD_SE_CANON = {'Y': 'Y', 'S': 'S', 'F': 'F', 'Q': 'Q', 'M': 'M', 'D': 'D'}
...
def aggregate_period(data_rows, prd_se, target_period, method, span=None):
    ...
    if len(target_period) == 4 and prd_se in {'M', 'Q'} and method == 'sum':
        expected_count = 12 if prd_se == 'M' else 4
        distinct_periods = {str(row.get('PRD_DE', '')) for row in matching}
        if len(distinct_periods) != expected_count:
            return None, ''
...
def derive_actual(data_rows, prd_se, period, row):
    ...
    mapping_type = normalize_mapping_type(row.get('mapping_type'))
...
def verify_row(
    row, meta_cache, delay, use_pinned_item=True, data_cache=None,
    ...
    if (
        not row.get('mapping_status')
        and str(row.get('candidate_rank', '')).strip() != '1'
        and not diagnostic
    ):
    ...
    if key not in meta_cache:
        if request_limiter is None:
            ...
            meta_cache[key] = get_meta(
                org_id, tbl_id, 'ITM', request_limiter=request_limiter,
            )
        if request_limiter is None:
            time.sleep(delay)
    ...
    if not has_obj_axis:
        return mark_unverifiable(
            out, 'MISSING_OFFICIAL_AXIS', 'metadata',
            'KOSIS Param API 필수 objL1 공식 코드가 메타데이터에서 확인되지 않음',
        )
    ...
    obj_l1 = str(row.get('selected_obj_l1', '')).strip() or None
    obj_reason = (
        f"objL1={row.get('selected_obj_l1_name','')}[{obj_l1}] "
        f"axis={row.get('selected_obj_l1_axis_id','')}"
    )
    claim_prd_se = normalize_prd_se(row.get('prd_se'), row.get('period'))
    prd_se = str(row.get('api_prd_se') or claim_prd_se).strip().upper()
    if claim_prd_se == 'Y' and prd_se in {'M', 'Q'}:
        aggregation = str(row.get('period_aggregation') or '').strip().lower()
        if aggregation not in {'sum', 'latest'}:
            return mark_unverifiable(
                out, 'ANNUAL_AGGREGATION_UNSPECIFIED', 'period',
                '연간 값을 월/분기 자료로 계산할 sum/latest 의미가 확정되지 않음')
    if not str(row.get('prd_se', '')).strip() and claim_prd_se:
        note = f"prd_se 미지정 → period 형식에서 '{claim_prd_se}' 추론 [위험도 중간]"
    ...
    try:
        ...
        if obj_l1:
            request_fields['objL1'] = obj_l1
        request_fields.update({
            f'objL{level}': value
            for level in range(2, 9)
            if (value := selected_obj_params.get(f'obj_l{level}'))
        })
        out['api_request_fields'] = canonical_stat_request(request_fields)
        out['api_request_fingerprint'] = stat_request_fingerprint(request_fields)
        expected_fingerprint = str(row.get('api_request_fingerprint') or '').strip()
        if expected_fingerprint and expected_fingerprint != out['api_request_fingerprint']:
            return mark_unverifiable(
                out, 'REQUEST_FINGERPRINT_MISMATCH', 'contract',
                f"validator={expected_fingerprint} verifier={out['api_request_fingerprint']}")
        data = get_stat_data(
            org_id=org_id,
            tbl_id=tbl_id,
            obj_l1=obj_l1,
            ...
            request_cache=data_cache,
            request_limiter=request_limiter,
            axisless_table=not has_obj_axis,
            **selected_obj_params,
            **prd_params,
        )
        if request_limiter is None:
        ...
        return mark_unverifiable(
            ...
            'KOSIS_API_ERROR',
            'api',
            f'KOSIS data API 오류: {safe_exception_message(exc)}',
            kosis_obj_l1=obj_l1,
    ...
    non_error_rows = [row for row in data if not row.get('err')]
    if not non_error_rows:
        return mark_unverifiable(
            out, 'API_EMPTY_RESPONSE', 'api',
            'KOSIS API가 요청 좌표에 대해 데이터 행을 반환하지 않음',
            kosis_obj_l1=obj_l1,
            kosis_itm_id=item.get('ITM_ID', ''),
            kosis_itm_name=item.get('ITM_NM', ''),
            kosis_prd_se=prd_se,
        )
    coordinate_rows = exact_api_coordinate_rows(non_error_rows, request_fields)
    if not coordinate_rows:
        return mark_unverifiable(
            out, 'API_COORDINATE_RESPONSE_MISMATCH', 'api',
            'KOSIS API 응답의 ITEM/C1~C8/주기가 요청 좌표와 일치하지 않음',
            kosis_obj_l1=obj_l1,
            kosis_itm_id=item.get('ITM_ID', ''),
            kosis_itm_name=item.get('ITM_NM', ''),
            kosis_prd_se=prd_se,
        )
    data_rows = clean_data_rows(coordinate_rows)
    if not data_rows:
        return mark_unverifiable(
            out, 'API_NON_NUMERIC_VALUE', 'api',
            '요청 좌표의 KOSIS 응답에 비교 가능한 수치 DT가 없음',
            kosis_obj_l1=obj_l1,
            kosis_itm_id=item.get('ITM_ID', ''),
            kosis_itm_name=item.get('ITM_NM', ''),
            kosis_prd_se=prd_se,
    ...
    if mapping_type == 'rate_from_level':
        ...
        and canonicalize_unit(kosis_unit_value) == '%'
        and canonicalize_unit(row.get('unit')) == '%p'
    ):
        factor, unit_reason = 1.0, '비율 수준값의 차이를 %p로 계산'
        compatible, compatible_reason = actual_raw is not None, ''
    else:
    ...
    if actual_raw is None:
        ...
        verdict_code, verdict_stage = 'SEMANTIC_SCOPE_REVIEW_REQUIRED', 'semantic'
    else:
...
def main():
    ...
    parser.add_argument(
        '--meta-index', default='',
        help='Preloaded official metadata CSV; avoids repeated getMeta calls.',
    )
    parser.add_argument(
        '--sqlite-cache-db', default='',
        help='Persistent successful KOSIS API response cache.',
    )
    ...
    parser.add_argument(
        '--allow-item-reselection', action='store_true',
        help='진단용: selected_itm_id 대신 verifier가 ITEM을 다시 선택한다.')
    ...
    meta_cache = {}
    if args.meta_index:
        for meta_row in read_csv(Path(args.meta_index).expanduser())[0]:
            key = (
                str(meta_row.get('org_id') or meta_row.get('ORG_ID') or '').strip(),
                str(meta_row.get('tbl_id') or meta_row.get('TBL_ID') or '').strip(),
            )
            if all(key):
                meta_cache.setdefault(key, []).append({
                    "OBJ_ID": meta_row.get("axis_id") or meta_row.get("OBJ_ID") or "",
                    "OBJ_NM": meta_row.get("axis_name") or meta_row.get("OBJ_NM") or "",
                    "OBJ_ID_SN": meta_row.get("axis_order") or meta_row.get("OBJ_ID_SN") or "",
                    "ITM_ID": meta_row.get("code_id") or meta_row.get("ITM_ID") or "",
                    "ITM_NM": meta_row.get("code_name") or meta_row.get("ITM_NM") or "",
                    "UP_ITM_ID": (
                        meta_row.get("parent_code_id") or meta_row.get("UP_ITM_ID") or ""
                    ),
                    "UNIT_ID": meta_row.get("unit_id") or meta_row.get("UNIT_ID") or "",
                    "UNIT_NM": meta_row.get("unit_name") or meta_row.get("UNIT_NM") or "",
                    "UNIT_ENG_NM": (
                        meta_row.get("unit_eng_name") or meta_row.get("UNIT_ENG_NM") or ""
                    ),
                })
    if args.sqlite_cache_db:
        from kosis_metadata_store import SQLiteAPIResponseCache, SQLiteKosisRequestLimiter
        data_cache = SQLiteAPIResponseCache(args.sqlite_cache_db)
        request_limiter = SQLiteKosisRequestLimiter(
            args.sqlite_cache_db, interval=max(float(args.delay), 0.5),
        )
    else:
        data_cache = {}
        request_limiter = None
    ...
    for i, row in enumerate(rows, 1):
        try:
            verified = verify_row(
                ...
                meta_cache,
                args.delay,
                use_pinned_item=not args.allow_item_reselection,
    ...
    extra_fields = [
        ...
        'api_request_fields', 'api_request_fingerprint',
