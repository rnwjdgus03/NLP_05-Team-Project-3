...
- 처리:
  ...
  - 파라미터가 바뀐 행 중 API 조회 가능한 69건만 KOSIS API로 재조회했고, 15건 성공 / 54건 실패 또는 세부 코드 미확정.
  - 최종 거친 점검 기준에서 비검증인데 `tbl_id`가 남은 행, GDP인데 GDP표가 아닌 행, 무역/물가 도메인이 뒤섞인 행은 0건으로 정리됨.
- 재매핑 결과:
  - 전체: 2,001건
  - `tbl_id` 공백: 733건
  - `검증완료_일치`: 108건
  ...
  - `판단불가_API조회실패`: 24건
- 해석:
  - 일치 건수는 108건으로 줄었지만, 잘못된 `tbl_id`를 억지로 남기지 않게 하여 매핑 품질은 개선됨.
  - `tbl_id`가 비어 있는 733건은 "KOSIS 후보가 아예 없다"가 아니라, 현재 자동 규칙으로 확정하면 오매칭 위험이 커서 후보 재검색/수동확인이 필요한 행임.
