...
# A company-focused article can quote an independent national statistic in a
# later paragraph.  The title must not leak into that claim's subject.  We
# only waive the title-level company rule when the sentence itself names a
# public statistics producer and contains no company from ``SINGLE_COMPANY``.
OFFICIAL_AGGREGATE_SOURCE = (
    "국가데이터처", "통계청", "산업통상자원부", "관세청", "한국은행",
    "고용노동부", "국토교통부", "농림축산식품부", "기획재정부",
)
AGGREGATE_STATISTIC_SUBJECT = (
    "수출", "수입", "무역", "물가", "고용", "취업", "실업", "인구",
    "출생", "사망", "생산", "소비", "판매", "전국", "우리나라",
)
...
def single_company_metric(claim_text: str, row: Mapping[str, Any]):
    ...
    if title_company and metric:
        official_source = _has(claim_text, OFFICIAL_AGGREGATE_SOURCE)
        aggregate_subject = _has(
            " ".join((claim_text, _first(row, "measurement_indicator", "indicator"))),
            AGGREGATE_STATISTIC_SUBJECT,
        )
        company_in_claim = _has(claim_text, SINGLE_COMPANY)
        if official_source and aggregate_subject and not company_in_claim:
            # Claim-level provenance overrides unrelated company words in the
            # article title.  Later checks still reject forecasts, converted
            # currencies, non-KOSIS sources, and other unsafe measurements.
            return None
        return (
