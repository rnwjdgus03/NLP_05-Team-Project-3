...
현재 `kosis_table_summary.csv`에는 30개 최상위 카테고리 전체가 크롤링되어, 총 96,018개 통계표가 인덱싱되어 있습니다.
...
- **KOSIS 인덱스/메타 조회**: 96,018개 통계표 전체 인덱싱 완료, 메타정보 조회 스크립트 완료.
...
- **B팀 1차 필터링/매칭**: `is_claim=True` 중 KOSIS 검증 가능성이 낮은 metric(날짜·시간/증시지표/기업실적/정책·제도/환율/인명피해)을 제외한 6,404건을 KOSIS 매칭 대상으로 확정.
  - 이 중 2,001건은 `tbl_id`까지, 1,998건은 `obj_l1`/`itm_id`까지 자동/반자동으로 채워짐 (`outputs/bteam_review/bteam_kosis_review_filled.csv`).
  - 나머지 4,403건은 수동 검토 필요 (`outputs/bteam_review/bteam_kosis_review_manual_todo.csv`) — 현재 진행 중인 작업.
...
- **다음 할 일**:
  1. `bteam_kosis_review_manual_todo.csv`(4,403건) 검토 — 김진성/구정현 분담 방식 협의 중.
  2. 확정된 매핑에 `prd_se`(연/분기/월) 채우기.
  3. `verify_claim.py`로 확정된 배치 검증 실행, 결과 취합.
  4. A팀에 `time`(구조화된 시점), 개별기업/해외기업 claim 제외 등 추가 정제 요청 검토.
