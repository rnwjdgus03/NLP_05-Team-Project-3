...
**결정**: 검색은 **lexical 단독**을 기본으로 합니다. 임베딩 인덱스(BGE-M3·KURE)·Colab·리랭커는 이 도메인에서 이득이 없어 필수 경로에서 제외합니다(코드·인덱스는 재현·재검토용으로만 보존). 남은 recall 갭(37.5%)은 임베딩이 아니라 **키워드 확장 / KOSIS 통합검색 API / 코드북**으로 접근합니다.

> 주의: 파일럿 표본이 24건(정답표 라벨 기준)으로 방향성 결론입니다. 골드 확장 시 재검증합니다.
