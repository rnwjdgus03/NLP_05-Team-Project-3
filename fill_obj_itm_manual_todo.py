"""
bteam_kosis_review_manual_todo.csv 중 org_id/tbl_id는 확정됐지만
obj_l1/itm_id/prd_se가 비어있는 행에 대해, KOSIS 메타정보 API로 실제 분류축/항목 코드를
조회해서 채워주는 스크립트.

fill_obj_itm.py와 동일한 로직이며, 대상 파일만 manual_todo.csv로 바꾼 버전.

*** 반드시 KOSIS API(kosis.kr)에 접속 가능한 로컬 환경(PowerShell)에서 실행할 것 ***

동작 방식:
  - claude(또는 사람)가 candidate_kosis_table 후보 중 맞는 표를 판단해서
    org_id/tbl_id를 미리 채워둔 행만 대상으로 함 (org_id/tbl_id가 비어있으면 건너뜀)
  ...
  - prd_se(수록주기: Y=연간, H=반기, Q=분기, M=월별)는 이 스크립트가 채우지 않음 -> claim_text의
    시점 표현을 보고 직접 채워야 함 (단, 이미 채워져 있으면 건드리지 않음)

재실행해도 안전: 이미 obj_l1/itm_id가 채워진 값은 덮어쓰지 않음.

사용법:
    python fill_obj_itm_manual_todo.py
"""
...
def main():
    ...
    print(f"\n완료 -> {PATH}")
    print(f"obj_l1/itm_id 자동 완료: {auto_filled}건, 수동 선택 필요: {need_manual}건, "
          f"이미 완료돼서 건너뜀: {skipped_done}건, org_id/tbl_id 미확정이라 건너뜀: {skipped_no_table}건")
    print("주의: prd_se(Y/H/Q/M)는 이 스크립트가 채우지 않음. claim_text 시점 보고 직접 채워야 함.")
