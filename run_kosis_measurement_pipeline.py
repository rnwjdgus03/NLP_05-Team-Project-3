...
KOSIS target.  It prepares an verification_input_ready measurement file first and only sends a
...
def validate_reusable_candidates(ready_path, candidate_path):
    ...
    if len(top1) != len(expected):
        raise ValueError(
            "재사용 후보 CSV의 1위 후보 수가 verification_input_ready measurement 수와 다릅니다. "
...
def main():
    ...
    parser.add_argument("--top-tables", type=int, default=20, help="Table hybrid candidate Top-K before reranking/compression")
    parser.add_argument("--top-rank-for-meta", type=int, default=5, help="Reranked table Top-K to fetch metadata")
    ...
    parser.add_argument(
        ...
        default="hybrid",
        help="기본은 keyword + BGE-M3 hybrid 검색. 재현성/오프라인 비교가 필요하면 lexical 지정",
    )
    parser.add_argument("--semantic-index", default="data/indexes/kosis_bge_m3")
    parser.add_argument("--period-route-index", default="data/claims/kosis_period_table_routes.csv")
    ...
    parser.add_argument("--max-combinations", type=int, default=5, help="API 직전 후보 Top-K. debug/recall 모드에서만 20 권장")
    parser.add_argument(
        "--api-sample-limit",
        type=int,
        default=0,
    ...
    parser.add_argument("--chroma-coordinate-index", default="", help="Optional Chroma coordinate index for ITEM/OBJ retrieval")
    ...
    parser.add_argument("--chroma-validation-top-k", type=int, default=20, help="Coordinate candidate Top-K")
    ...
    verification_input_ready = out_dir / f"{stem}_verification_input_ready.csv"
    rejected = out_dir / f"{stem}_verification_input_rejected.csv"
    ...
    if args.reuse_table_candidates:
        validate_reusable_candidates(verification_input_ready, table_candidates)
    else:
    ...
    if args.verify:
        validate_command = [
            ...
            args.max_combinations,
            "--chroma-dense-top-k",
            args.chroma_dense_top_k,
            "--chroma-lexical-top-k",
            args.chroma_lexical_top_k,
            "--chroma-rerank-top-k",
            args.chroma_rerank_top_k,
            "--chroma-validation-top-k",
            args.chroma_validation_top_k,
            "--chroma-max-documents",
            args.chroma_max_documents,
            "--api-sample-limit",
            args.api_sample_limit,
            "--delay",
            args.delay,
        ]
        if args.chroma_coordinate_index:
            validate_command.extend(["--chroma-coordinate-index", args.chroma_coordinate_index])
        if args.no_chroma_reranker:
            validate_command.append("--no-chroma-reranker")
