...
- 입력 컬럼 자동 인식: 한글 형식(문장/전_문장/전전_문장/작성일) 또는 표준 형식(claim_text/prev_sentence/prev_prev_sentence/...)
...
PROMPT_VERSION = "isclaim-v1.3-prev2-context"
...
- 이전 두 문장과 기사 작성일은 현재 문장의 생략된 지표명·시점·대상·비교 기준을 해석하는 데 사용한다.
[False 조건]
...
- 이전 문장들만 검증 가능하고 현재 문장은 검증 불가능하다.
...
def call_hcx(api_key, model, text, prev, prev_prev, date="-", retries=4):
    body = {
        "messages": [
            ...
            try:
                if not text:
                    ...
                    j = call_hcx(key, a.model, text,
                                 r.get(cols.get("prev_sentence", ""), "-"),
                                 r.get(cols.get("prev_prev_sentence", ""), "-"),
