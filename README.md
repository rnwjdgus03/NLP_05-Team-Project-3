...
현재 `kosis_table_summary.csv`에는 30개 최상위 카테고리 전체(농림·인구·노동·물가·사회일반·소득소비자산·경제일반경기·무역국제수지·임금·도소매서비스·범죄안전·보건·복지·교육훈련·문화여가·주거·국토이용·기업경영·수산·광업제조업·건설·교통물류·정보통신·과학기술·국민계정·정부재정·금융·환경·에너지·지역통계)가 크롤링되어, 총 96,018개 통계표가 인덱싱되어 있습니다.
...
- B팀: KOSIS API 연동, 통계표 인덱스(96,018개, 30개 카테고리 전체 완료), 메타정보 조회, 파라미터 가이드, 자동 매칭 스크립트까지 준비 완료
- A팀: 노션 페이지에 조선일보 원본 기사 코퍼스(2,706건, 2025.01.01~12.25, 검색 구분 레이블 True/False)가 공개됨. 아직 claim_candidates.csv(claim_id, claim_text, metric, time, population, unit) 형식으로 가공되기 전 단계라, 그 결과물이 나오면 B팀 매칭 스크립트를 바로 돌릴 수 있음
...
- 2026.07.13 추가: 범죄·안전(C), 보건(F), 복지(G), 교육·훈련(H1), 문화·여가(H2), 주거(I1), 국토이용(I2), 기업경영(J2), 수산(K2), 광업·제조업(L), 건설(M1), 교통·물류(M2), 정보통신(N1), 과학·기술(N2), 국민계정(Q) 카테고리 크롤링 완료 (총 86,060건)
- 2026.07.13 추가: 남은 정부·재정(R), 금융(S1), 환경(T), 에너지(U), 지역통계(V) 카테고리까지 크롤링 완료 — 30개 최상위 카테고리 전체 인덱싱 완료, 총 96,018건
- 2026.07.13 추가: 노션 페이지에 A팀 원본 기사 코퍼스(조선일보, 2,706건) 공개됨. claim_candidates.csv로 가공되기 전 단계라 매칭 스크립트는 아직 대기 중
