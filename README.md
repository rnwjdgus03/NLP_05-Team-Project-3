...
같은 READY 39건, 잠긴 골드셋, 공식 메타/API 검증 조건으로 lexical과 BGE를 직접
비교한 결과는 다음과 같다.

| 검색 방식 | 최소 포화 K | TBL recall | 기술 유효 후보 | READY | ITEM/OBJ 골드 hit |
|---|---:|---:|---:|---:|---:|
| **lexical** | **Top-2** | **15/24 (62.5%)** | 18 | 0 | 0/20 |
| BGE-M3 hybrid | Top-2 | 14/24 (58.3%) | 19 | 0 | 0/20 |

따라서 **통계표 검색은 lexical Top-2**로 확정한다. Top-3·5에서도 lexical 정답표가
추가되지 않았고 BGE보다 한 건 더 찾았으므로, 임베딩과 리랭커를 운영 필수 경로에
넣을 근거가 없다. 단, 두 방식 모두 READY가 0건이므로 이 결정은 검색 단계에만
적용한다. ITEM/OBJ 자동 확정과 verdict 배포는 코드북·코드셋·비교기간 규칙을 보강한
뒤 다시 평가한다.

재현 노트북은 `notebooks/kosis_lexical_vs_bge_colab.ipynb`, 상세 보고서는
[`docs/kosis_lexical_vs_bge_result_20260723.md`](docs/kosis_lexical_vs_bge_result_20260723.md),
공개 집계는
[`docs/results/kosis_lexical_vs_bge_topk_summary_20260723.csv`](docs/results/kosis_lexical_vs_bge_topk_summary_20260723.csv)에 있다.
