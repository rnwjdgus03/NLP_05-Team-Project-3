...
## 출력 JSON 스키마
{
 ...
 "gender": "남|여|전체|-",
 "industry_or_item": "산업/품목명. [검증 대상 문장]에 나오면 반드시 쓰고, 제목·앞뒤 문장에만 있으면 -",
 ...
 "measurements": [  // 의미가 있는 수치마다 하나. KOSIS 검증 불가여도 버리지 않는다
   {"measurement_role": "현재값|이전값|증감값|증감률|목표값|참고값",
    ...
    "measurement_item": "이 수치가 가리키는 품목·산업·대상. [검증 대상 문장]에 나오면 반드시 쓰고(예: 석유화학 수출→석유화학), 제목·앞뒤 문장에만 있으면 -",
...
- 품목은 [검증 대상 문장]에 나오면 **반드시 쓰고**, 나오지 않으면 쓰지 않는다. 양쪽 다 지킨다.
  · 반드시 쓴다: '석유화학 수출은 480억 달러로' → item=석유화학
                 '선박 수출은 두 자릿수(18%) 증가한 256억 달러' → item=선박
                 '반도체가 43.9% 증가한 1419억 달러' → item=반도체
  · 쓰지 않는다: 제목이 '반도체가 이끈 수출'이어도
                 '전체 수출액이 6838억달러로 8.2% 증가' 문장은 item=-
  (실측 오류 두 가지. 문장에 없는 '반도체'가 붙어 전체 수출액 6838억을 반도체 1420억과
   대조했고, 반대로 규칙을 세게 쓰자 문장에 있는 석유화학·선박·바이오헬스까지 지워졌다.)
- 앞 문장의 대상을 이어받는 생략은 허용한다. 다만 그때는 대상을 measurement_indicator에 남긴다.
  예: '이들이 차지하는 비율도 1.5%에서 4.4%로' → indicator='조선 산업기술인력 중 외국인 비율', item='-'
