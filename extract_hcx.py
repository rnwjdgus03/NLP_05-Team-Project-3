...
## 출력 JSON 스키마
{
 ...
 "measurements": [  // 의미가 있는 수치마다 하나. KOSIS 검증 불가여도 버리지 않는다
   {"measurement_role": "현재값|이전값|증감값|증감률|목표값|참고값",
    ...
    "measurement_item": "품목·산업·대상 (예: 반도체, 바이오헬스). 문장이 총계를 말하면 -",
...
- 그 문장이 **총계·전체**를 말하면 item은 -다. 기사 제목이 특정 품목을 다뤄도 마찬가지다.
  예: 제목이 '반도체가 이끈 수출'이어도
      '전체 수출액이 6838억달러로 8.2% 증가' 문장은 item=-
  (실측 오류: 이 문장에 item=반도체가 붙어 전체 수출액 6838억을 반도체 1420억과 대조했다.)
  ※ 이 규칙은 **총계 문장에만** 적용한다. 품목이 문장에 나오면 그대로 쓴다 —
    '전기차 80만대', '로봇이 주요 공정의 100%', 'LCC 이용객 2419만명' 은 모두 item을 쓴다.
