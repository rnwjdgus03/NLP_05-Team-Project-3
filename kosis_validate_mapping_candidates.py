...
def find_high_risk_missing(claim: Mapping[str, Any] | None,
    ...
    claim_text = _normalized_text(" ".join(str(_first(claim, key)) for key in (
        "indicator", "measurement_indicator", "industry_or_item", "measurement_item",
        "claim_text", "measurement_text", "origin_country", "destination_country",
    )))
    if any(token in claim_text for token in ("전망", "전망치", "예상", "예측", "추산", "추정", "될것으로")):
        missing.append("forecast_or_estimate_not_observed")
    country_aliases = {
        "china_scope": ("대중", "중국", "對中"),
        "us_scope": ("대미", "미국", "對美"),
        "japan_scope": ("대일", "일본", "對日"),
        "eu_scope": ("대eu", "유럽연합", "eu"),
    }
    for scope_name, aliases in country_aliases.items():
        normalized_aliases = [token for token in (_normalized_text(alias) for alias in aliases) if token]
        alias_hit = any(token in claim_text for token in normalized_aliases)
        selected_hit = any(token in selected_normalized for token in normalized_aliases)
        if alias_hit and not selected_hit:
    ...
    for canonical, aliases in (
        ...
        if normalized_value and normalized_value not in {"", "-"} and normalized_value not in selected_normalized:
