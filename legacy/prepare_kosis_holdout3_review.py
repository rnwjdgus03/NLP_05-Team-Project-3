"""Apply the frozen codebook v3 to the fresh holdout3 sample and prepare the
gold review template. Mirrors prepare_kosis_holdout2_review.py exactly, but
points at kosis_codebook_v3 (the version whose backlog rules were just added
and frozen) and the outputs/bteam_holdout3 sample.

Run this locally (needs a working KOSIS_API_KEY + network access) after
select_kosis_holdout3_100.py has produced holdout3_100_selection.csv. Once it
finishes, a human must fill in every gold_* column in holdout3_100_review.csv
before the 80% gate can be computed -- auto_* columns are v3's predictions,
not ground truth.
"""
...
OUTPUT_DIR = REPO / "outputs/bteam_holdout3"
INPUT = OUTPUT_DIR / "holdout3_100_selection.csv"
REVIEW_OUTPUT = OUTPUT_DIR / "holdout3_100_review.csv"
REPORT_OUTPUT = OUTPUT_DIR / "holdout3_100_review_report.md"
codebook.CACHE_PATH = OUTPUT_DIR / "holdout3_auto_api_cache.json"
...
def main():
    ...
    if len(selection) != 100:
        raise RuntimeError(f"holdout3 rows={len(selection)}")
    ...
    report = [
        "# KOSIS 코드북 v3 3차 독립 표본 검토 준비",
        "",
        "## 상태",
        "",
        "- 골드100·홀드아웃100·홀드아웃2_100 총 300건과 claim_id·article_id·claim_text 중복 0건",
        f"- 표본: {len(rows)}건, 분야별 {dict(domain_counts)}",
        f"- 모집단 출처: {dict(source_counts)}",
        f"- 코드북 v3 자동 결정: {dict(decision_counts)}",
        f"- 자동 매핑 API 결과: {dict(api_counts)}",
        "- 현재 파일의 gold_* 컬럼은 비어 있으며 사람이 확정하기 전까지 품질 게이트를 계산하지 않는다.",
        "- v3는 이 표본을 만들기 전에 동결되었다 (65건 백로그 중 6건만 규칙 반영, 나머지는 verify() 엔진 확장 필요로 보류).",
        "",
        "## 수동 확정 순서",
        "",
        ...
        "5. 100건을 모두 확정한 뒤 코드북 v3를 수정하지 않은 상태로 80% 게이트를 계산한다.",
        "",
        "## 파일",
        "",
        "- `holdout3_100_selection.csv`: 새 독립 표본",
        "- `holdout3_100_review.csv`: 자동 예측 + 수동 골드 입력용",
        "- `holdout3_auto_api_cache.json`: 자동 매핑 API 캐시",
    ]
