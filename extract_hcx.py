...
## 출력 JSON 스키마
{
 ...
 "gender": "남|여|전체|-",
 "industry_or_item": "산업/품목명. **[검증 대상 문장]에 실제로 나올 때만** 쓴다. 없으면 -",
 ...
 "measurements": [  // 의미가 있는 수치마다 하나. KOSIS 검증 불가여도 버리지 않는다
   {"measurement_role": "현재값|이전값|증감값|증감률|목표값|참고값",
    ...
    "measurement_item": "이 수치가 가리키는 품목·산업·대상. **그 대상이 [검증 대상 문장]에 나올 때만** 쓴다. 제목·앞뒤 문장에만 있으면 -",
...
- **품목은 [검증 대상 문장]에 나올 때만 쓴다. 제목이나 앞뒤 문장의 품목을 끌어오지 않는다.**
  기사 전체가 한 품목을 다뤄도, 그 문장이 총계를 말하면 item은 -다.
  예: 제목이 '반도체가 이끈 수출'이어도 '전체 수출액이 6838억달러로 8.2% 증가' 문장의 item은 -다.
  (실측 오류: 이 문장에 item=반도체가 붙어 전체 수출액 6838억을 반도체 수출액 1420억과 대조했다.)
