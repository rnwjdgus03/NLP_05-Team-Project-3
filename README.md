...
운영·평가 기본값은 재현성을 위해 `--retrieval-mode lexical`입니다. BGE-M3 임베딩
비교 실험을 할 때만 `--retrieval-mode hybrid`를 명시적으로 사용합니다. `hybrid`는
인덱스나 ML 의존성이 없을 때 즉시 실패하므로 시연·평가 환경 확인에 적합합니다.
`--no-reranker`를 주면 임베딩과 규칙 검색만 결합합니다.
...
`in_ready=Y`와 `mapping_status=READY`는 서로 다릅니다.

| 구분 | 의미 | 수치 검증 여부 |
|---|---|---|
| `in_ready=Y` | KOSIS로 검증해볼 만한 measurement | 아직 수치 검증하지 않음 |
| `mapping_status=READY` | 공식 메타와 실제 API exact match를 통과한 확정 매핑 | 수치 검증 가능 |

즉 `in_ready=Y` 전체를 바로 수치 검증하지 않고, 잘못된 ITEM/OBJ 조합으로 인한 오판정을 막기 위해 `mapping_status=READY`인 후보만 실제값 비교에 사용합니다.
...
- 검색 기본값은 `lexical`로 고정하고, hybrid/BGE 비교는 명시적으로 실행합니다.
