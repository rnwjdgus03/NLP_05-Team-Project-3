...
BIG_MAGNITUDES = {key: MAGNITUDES[key] for key in ("조", "억", "만")}
SMALL_MAGNITUDES = {key: MAGNITUDES[key] for key in ("천", "백")}
...
def normalize_number(value):
    ...
    try:
        if not any(magnitude in text for magnitude in MAGNITUDES):
            ...
            # Korean large units multiply the whole section before them.
            # For example, 5천121만 is (5,000 + 121) * 10,000, not
            # 5,000 + 121 * 10,000.  Treat 조/억/만 as section delimiters
            # and 천/백 as multipliers inside each section.
            tokens = []
            position = 0
            ...
            previous_big: Decimal | None = None
            previous_small: Decimal | None = None
            for token in tokens:
                if token not in MAGNITUDES:
                    if pending is not None:
                        raise InvalidOperation
                    pending = Decimal(token)
                    continue

                if token in SMALL_MAGNITUDES:
                    magnitude = SMALL_MAGNITUDES[token]
                    if pending is None or (previous_small is not None and magnitude >= previous_small):
                        raise InvalidOperation
                    section += pending * magnitude
                    pending = None
                    previous_small = magnitude
                    continue

                magnitude = BIG_MAGNITUDES[token]
                if previous_big is not None and magnitude >= previous_big:
                    raise InvalidOperation
                if pending is not None:
                    section += pending
                    pending = None
                if section == 0:
                    raise InvalidOperation
                number += section * magnitude
                section = Decimal("0")
                previous_big = magnitude
                previous_small = None
...
def extract_numeric_candidates(text):
    ...
    for candidate in candidates:
        if candidate["measurement_usage"] == "DATE_OR_ORDER":
            continue
        # A numeric occurrence is the identity here, not its semantic value.
        # The same value can legitimately appear more than once for different
        # months, regions or items in a single claim (for example, "1월 10명,
        # 2월 10명").  Value-based deduplication silently removed the latter
        # occurrence before HCX could bind it to the correct period.
        key = (
            candidate["start"],
            candidate["end"],
            candidate["value"],
...
def normalize_hcx_measurements(result, candidates, text=""):
    # Preserve every source occurrence. A dict of `(value, unit) -> candidate`
    # used to bind all equal observations to the last occurrence, mixing
    # usage/approximation/context across periods or categories.
    candidate_by_key = {}
    for candidate in candidates:
        candidate_by_key.setdefault(
            (candidate["value"], candidate["unit"]), []
        ).append(candidate)
    consumed_candidate_ids = set()

    def select_candidate(key, measurement_text):
        pool = [
            candidate for candidate in candidate_by_key.get(key, [])
            if id(candidate) not in consumed_candidate_ids
        ]
        if measurement_text != "-":
            matching = [
                candidate for candidate in pool
                if candidate["measurement_text"] in measurement_text
                or measurement_text in candidate["measurement_text"]
            ]
            if matching:
                pool = matching
        if not pool and measurement_text != "-":
            pool = [
                candidate for candidate in candidates
                if id(candidate) not in consumed_candidate_ids
                and (
                    candidate["measurement_text"] in measurement_text
                    or measurement_text in candidate["measurement_text"]
                )
            ]
        candidate = pool[0] if pool else {}
        if candidate:
    ...
    for measurement in result.get("measurements") or []:
        ...
        measurement_text = norm(measurement.get("measurement_text"))
        candidate = select_candidate(key, measurement_text)
        if candidate:
        ...
        item["_source"] = "hcx"
        # Keep equal-valued observations when their grounded measurement
        # identity differs.  This is common in time series and category
        # comparisons.  Only exact semantic duplicates are collapsed.
        duplicate_key = (
            item["value"],
            item["unit"],
            norm(item.get("measurement_role")),
            norm(item.get("measurement_period")),
            norm(item.get("measurement_item")),
            norm(item.get("measurement_text")),
        )
