...
def test_prepare_preserves_previous_two_and_next_context_columns(tmp_path):
    source = tmp_path / "source.csv"
    row = measurement_row(
        prev_sentence="직전 문장이다.",
        prev_prev_sentence="이전이전 문장이다.",
        next_sentence="다음 문장이다.",
    )
    with source.open("w", encoding="utf-8-sig", newline="") as handle:
    ...
    with output.open(encoding="utf-8-sig", newline="") as handle:
        ...
        assert reader.fieldnames[
            reader.fieldnames.index("claim_text") + 1:
            reader.fieldnames.index("claim_text") + 4
        ] == ["prev_sentence", "prev_prev_sentence", "next_sentence"]

    assert output_row["prev_sentence"] == "직전 문장이다."
    assert output_row["prev_prev_sentence"] == "이전이전 문장이다."
    assert output_row["next_sentence"] == "다음 문장이다."
