...
def test_preprocess_articles_assigns_ids_and_sentence_context():
    ...
    assert rows[0]["prev_prev_sentence"] == ""
    assert rows[0]["next_sentence"] == "소비자물가는 2.3% 상승했다."
    assert rows[1]["prev_sentence"] == "통계청이 물가 동향을 발표했다."
    assert rows[1]["prev_prev_sentence"] == ""
    assert rows[1]["next_sentence"] == "전월보다는 0.2% 올랐다."
    assert rows[2]["prev_sentence"] == "소비자물가는 2.3% 상승했다."
    assert rows[2]["prev_prev_sentence"] == "통계청이 물가 동향을 발표했다."
    assert rows[2]["next_sentence"] == ""
    assert rows[3]["prev_sentence"] == ""
    assert rows[3]["prev_prev_sentence"] == ""
    assert rows[3]["next_sentence"] == ""
...
def test_cli_creates_hcx_input_csv(tmp_path):
    ...
    assert rows[1]["prev_sentence"] == "출생아 수가 3% 증가했다."
    assert rows[1]["prev_prev_sentence"] == ""
