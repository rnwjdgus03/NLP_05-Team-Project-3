...
def apply_candidate_scores(
    ...
    structural_bonus = (
        MAX_STRUCTURAL_BONUS if candidate.get("mandatory_aggregate") else 0.0
    )
...
def _is_aggregate(row: Mapping[str, Any]) -> bool:
    ...
    if explicit in {"1", "true", "y", "yes"}:
        return True
    return _text(row.get("obj_name")) in AGGREGATE_OBJ_NAMES
...
def retain_ranked_components(
    ...
    *,
    ...
    for raw in rows:
        ...
        # proximity and must survive the beam cutoff.
        bonus = 1.0 if target_match else (0.02 if aggregate and not target_terms else 0.0)
