...
따라서 현재 구조는 coverage를 무리하게 늘리기보다, 확실하지 않은 후보를 `NEEDS_CONFIRMATION`으로 남기고 READY 확정 결과의 정합성을 높이는 **precision-oriented mapping**입니다. 향후 과제는 이 READY 정교도를 유지하면서 HITL 보정값과 코드셋을 축적해 READY coverage를 높이는 것입니다.
