...
## 7. 게이트 2차 확장 (장중 시세 · 개별 기업)

남은 `NO_MATCH` 를 tier 별 좌표 재현율로 보면 `NEAR_MISS 61.5%` vs `NO_MATCH 28.9%` 로
갈렸다. 실재하는 통계일수록 재현율이 높다는 뜻이므로 `NO_MATCH` 에 범위 밖 주장이
더 있다고 보고 두 규칙을 추가했다.

| 규칙 | 판정 | 근거 |
|---|---|---|
| `INTRADAY_MARKET_RATE` / `DAILY_MARKET_RATE` | REJECT | KOSIS 는 월평균·연평균만 수록. '오후 3시 30분 기준 1466.6원' 을 월평균 표에 댈 수 없다 |
| `SINGLE_COMPANY_METRIC` | REJECT | KOSIS 는 통계법상 개별 기업 자료를 수록하지 않는다(산업 집계만) |
| `POSSIBLE_COMPANY_METRIC` | REVIEW | 문장에만 기업명이 있으면 대상이 산업 집계일 수 있어 버리지 않는다 |

**민간·협회 조사는 넣지 않았다.** 풀에 `중소기업실태조사`(250표),
`산업기술인력수급실태조사` 등이 실재해 오탐 위험이 크다.
...
| 확정 집합 | 130 | **125** |
| REVIEW(유지) | 9 | 29 |
| `NO_MATCH` 포착률 | 33.6% | **37.0%** |
| **오탐(SILVER_UNIQUE 차단)** | 0 | **0** |

`SINGLE_COMPANY_METRIC` 이 1건만 REJECT 된 것은 설계대로다 —
지표 필드에 기업명이 명시된 경우만 막고 나머지는 REVIEW 로 남긴다.
...
오염을 걷어낼수록 격차가 줄지만 **A 가 계속 앞선다.** 표본 축소에 따른 변동으로 보이며,
ChromaDB 종료 결론은 유지한다.
