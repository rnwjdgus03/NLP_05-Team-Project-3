...
from run_kosis_coordinate_stage_a import (
    ...
    period_within_metadata_range,
    metadata_table_score,
)
from run_kosis_coordinate_stage_c import (
    coordinate_scope_exact_match,
    diverse_table_top_k,
    item_indicator_exact_match,
...
def test_non_kosis_attributed_report_blocks_value_mismatch():
    row = exact_mismatch_row(
        claim_text="KB경영연구소의 2023 한국 반려동물 보고서에 따르면 반려가구는 552만 가구다.",
    )
    assert mismatch_claim_block_reasons(row) == [
        "claim_explicitly_attributed_to_non_kosis_source"
    ]
    blocked = block_unconfirmed_mismatch(row)
    assert blocked["verdict_code"] == "MISMATCH_BLOCKED_UNCONFIRMED_COORDINATE"


def test_range_or_approximate_value_blocks_value_mismatch():
    row = exact_mismatch_row(
        claim_text="2022년 이후 실업률이 2% 중후반대로 낮아졌다.",
    )
    assert "claim_value_is_range_or_approximation" in mismatch_claim_block_reasons(row)
    assert not mismatch_evidence_is_exact(row)


def test_explicit_kosis_point_value_can_still_support_mismatch():
    row = exact_mismatch_row(
        claim_text="국가통계포털 KOSIS에 따르면 2024년 수출액은 10달러다.",
    )
    assert mismatch_claim_block_reasons(row) == []
    assert mismatch_evidence_is_exact(row)
...
def test_stage_a_period_filter_rejects_known_mismatch_before_rerank():
    ...
    class Store:
        def table_rerank_metadata(self, _candidates):
            return [
                {"org_id": "1", "tbl_id": "Y", "periodicities": ["Y"],
                 "period_ranges": [{"prd_se": "Y", "range_text": "Y:2000~2025"}],
                 "metadata_complete": True},
                {"org_id": "1", "tbl_id": "M", "periodicities": ["M"],
                 "period_ranges": [{"prd_se": "M", "range_text": "M:2017.01~2026.06"}],
                 "metadata_complete": True},
...
def test_period_range_parser_is_conservative_and_exact():
    assert period_within_metadata_range("202606", "M", "M:2017.01~2026.06") is True
    assert period_within_metadata_range("202606", "M", "M:2001.01~2017.12") is False
    assert period_within_metadata_range("2025", "Y", "Y:2023~2025") is True
    assert period_within_metadata_range("2026", "Y", "Y:2003~2025") is False
    assert period_within_metadata_range("202604", "Q", "Q:2017 1/4~2026 2/4") is None


def test_stage_a_period_filter_rejects_out_of_range_same_periodicity():
    candidates = [
        {"org_id": "1", "tbl_id": "OLD", "table": {"tbl_name": "월간 거래액"}},
        {"org_id": "1", "tbl_id": "CURRENT", "table": {"tbl_name": "월간 거래액"}},
    ]

    class Store:
        def table_rerank_metadata(self, _candidates):
            return [
                {"org_id": "1", "tbl_id": "OLD", "periodicities": ["M"],
                 "period_ranges": [{"prd_se": "M", "range_text": "M:2001.01~2017.12"}],
                 "metadata_complete": True},
                {"org_id": "1", "tbl_id": "CURRENT", "periodicities": ["M"],
                 "period_ranges": [{"prd_se": "M", "range_text": "M:2017.01~2026.06"}],
                 "metadata_complete": True},
            ]

    kept, counts = apply_pre_rerank_metadata(
        candidates, {"prd_se": "M", "period": "202606", "indicator": "거래액"},
        Store(), minimum_candidates=1,
    )
    assert [row["tbl_id"] for row in kept] == ["CURRENT"]
    assert counts["period_range_mismatch_rejected"] == 1
    assert counts["period_range_exact"] == 1
...
def test_metadata_score_aligns_structured_obj_axis_with_table_axis():
    region_score, region_signals = metadata_table_score({
        "table": {
            "tbl_name": "행정구역별 주택매매거래현황",
            "category_path": "부동산거래현황",
        }
    }, {"region": "강남구", "indicator": "주택매매거래량"})
    other_score, other_signals = metadata_table_score({
        "table": {
            "tbl_name": "거래주체별 주택매매거래현황",
            "category_path": "부동산거래현황",
        }
    }, {"region": "강남구", "indicator": "주택매매거래량"})
    assert region_score - other_score >= 0.18
    assert "obj_axis:region" in region_signals
    assert "obj_axis:region" not in other_signals
...
def test_coordinate_scope_allows_item_bound_target_only_with_aggregate_objs():
    assert coordinate_scope_exact_match(
        ["남성"], "남자인구수", ["전국", "전체"],
    )
    assert not coordinate_scope_exact_match(
        ["남성"], "남자인구수", ["강남구", "전체"],
    )
    assert coordinate_scope_exact_match(
        ["서울"], "총인구수", ["서울특별시", "전체"],
...
def test_item_unit_text_recovers_area_count_mismatch():
    state = item_structure_state(
        {"unit_dimension": "count", "unit_dimension_hypotheses": "count"},
        {"unit": "천㎡", "unit_dimension": ""},
    )
    assert state["unit_structural_state"] == "mismatch"
    assert state["structural_compatible"] is False


def test_item_indicator_exact_match_removes_explicit_scope_and_average_only():
    assert item_indicator_exact_match(
        {"measurement_indicator": "평균 농가소득"}, "농가소득", [],
    )
    assert item_indicator_exact_match(
        {"measurement_indicator": "남성 사망자 수"}, "사망자 수", ["남성"],
    )
    assert not item_indicator_exact_match(
        {"measurement_indicator": "평균 농가소득"}, "농업소득", [],
    )
