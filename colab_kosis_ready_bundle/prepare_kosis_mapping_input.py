...
marked as verification_input_ready=Y and allowed into table discovery.  Rejected rows are retained with stable reason
...
def normalize_row(row: dict) -> dict:
    ...
    out["verification_input_ready"] = "Y" if not code else "N"
    out["mapping_eligible"] = out["verification_input_ready"]  # backward-compatible alias; not final mapping READY
...
def prepare(input_path: Path, output_path: Path, rejected_path: Path | None = None):
    ...
    accepted = [row for row in normalized if row["verification_input_ready"] == "Y"]
    rejected = [row for row in normalized if row["verification_input_ready"] != "Y"]
...
def main():
    ...
    print(f"input={len(accepted) + len(rejected)} verification_input_ready={len(accepted)} rejected={len(rejected)}")
    ...
    if args.expect_ready and len(accepted) != args.expect_ready:
        raise SystemExit(f"expected {args.expect_ready} verification_input_ready rows, got {len(accepted)}")
