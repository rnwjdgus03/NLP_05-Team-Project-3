...
| | 건수 | |
|---|---|---|
| 확정 | 9 (9%) | READY 또는 PROVISIONAL |
| 확인 불가 | 33 (32%) | 기간이 기사일 이후 15 · 좌표는 맞고 데이터 없음 12 · 기간 불명 6 |
| 시스템 한계 | 61 (59%) | 후보 비결정 25 · 단위 확인 불가 21 · 좌표 오류 12 · 대상 미근거 3 |

- **전체 기준 8.7%** — 사용자가 기사를 넣었을 때 실제로 답을 받을 확률
- **확인 가능 기준 12.9%** — 답할 수 있는 문제에서 시스템이 얼마나 하는가

확정이 12에서 9로 **줄어든 것이 개선**이다. 아래 「정밀도」 참고 —
빠진 3건 중 2건은 참인 기사를 '불일치'라고 말하고 있었다.
...
**확정이 줄어든 것이 개선이다.** 빠진 3건은 애초에 확정되면 안 되는 것이었고
그중 2건은 참인 기사를 거짓이라 말하고 있었다.

한계: 판정을 낸 것이 4건뿐이고 3건이 무역통계다. 넓은 주장은 할 수 없다.
