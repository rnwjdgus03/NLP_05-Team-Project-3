...
scope = _load("scope_v60", "kosis_scope_gate.py")
prepare = _load("prepare_v60", "prepare_kosis_mapping_input.py")
stage_a = _load("stage_a_v60", "run_kosis_coordinate_stage_a.py")
...
def test_stage_a_ignores_hcx_period_placeholders():
    row = {
        "measurement_period": "-",
        "measurement_prd_se": "-",
        "period": "202504",
    ...
    assert stage_a.requested_claim_period(row) == "202504"
    assert stage_a.requested_claim_periodicity(row) == "M"


def test_stage_a_keeps_real_measurement_period_over_article_period():
    row = {
        "measurement_period": "2024Q4",
        "measurement_prd_se": "Q",
        "period": "202504",
        "prd_se": "M",
    }
    assert stage_a.requested_claim_period(row) == "2024Q4"
    assert stage_a.requested_claim_periodicity(row) == "Q"
