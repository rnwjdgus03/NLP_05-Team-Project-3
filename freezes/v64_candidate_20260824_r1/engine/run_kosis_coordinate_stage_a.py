...
_MISSING_CLAIM_VALUES = {
    "", "-", "none", "null", "unknown", "n/a", "na", "미상", "없음",
}


def first_meaningful_claim_value(
    claim: Mapping[str, Any], *fields: str,
) -> str:
    """Return the first real value, ignoring HCX placeholder strings.

    HCX emits ``-`` for an unbound measurement period/periodicity.  A plain
    Python ``a or b`` treats that placeholder as truthy and can therefore hide
    the normalized article-level ``period``/``prd_se`` recovered upstream.
    """
    for field in fields:
        value = str(claim.get(field) or "").strip()
        if value.casefold() not in _MISSING_CLAIM_VALUES:
            return value
    return ""


def requested_claim_periodicity(claim: Mapping[str, Any]) -> str:
    return normalize_prd_se(first_meaningful_claim_value(
        claim, "measurement_prd_se", "prd_se", "claim_prd_se",
    ))


def requested_claim_period(claim: Mapping[str, Any]) -> str:
    return first_meaningful_claim_value(
        claim, "measurement_period", "period", "claim_period",
    )
...
def table_metadata_recall_terms(claim: Mapping[str, Any]) -> list[str]:
    """Build official survey/organization/table-family FTS query terms."""
    terms: list[str] = household_asset_taxonomy_terms(claim)
    for field in (
        "metric_domain", "survey_name", "source_survey", "statistics_name",
        "stat_name", "source_organization", "source_org", "publisher",
        "organization_name", "measurement_indicator", "indicator",
        "measurement_item", "industry_or_item",
    ):
        value = re.sub(r"\s+", " ", str(claim.get(field) or "")).strip(" -_/|,;()")
        if len(compact(value)) >= 2 and value not in terms:
            terms.append(value)
    for value in item_table_recall_terms(claim, structured_priority=True):
        if value not in terms:
            terms.append(value)
    return terms[:12]


def household_asset_taxonomy_terms(claim: Mapping[str, Any]) -> list[str]:
    """Map an explicit household-asset measure to public KOSIS taxonomy.

    The rule is label-free and applies before table IDs are known. Both the
    household population and asset metric must be present in the bound
    measurement indicator; a generic property-price article does not qualify.
    """
    indicator = compact(
        claim.get("measurement_indicator") or claim.get("indicator") or ""
    )
    if "가구" in indicator and "자산" in indicator:
        return ["가계금융복지조사", "입주형태별 자산 부채 소득 현황"]
    return []


def metadata_recall_enabled(
    claim: Mapping[str, Any], policy: str,
) -> bool:
    """Limit primary metadata recall to claims with an explicit trigger.

    The broad ``all`` mode is retained for the terminal metadata fallback.
    Primary retrieval uses ``household_asset`` so generic survey, organization,
    and indicator terms cannot perturb unrelated development examples.
    """
    if policy == "all":
        return True
    if policy == "survey_exact":
        # Explicit source metadata can open the bounded PostgreSQL table
        # metadata channel. A generic domain word cannot: metric_domain must
        # look like an official statistical series name.
        explicit = claim_metadata_values(
            claim,
            "survey_name",
            "source_survey",
            "statistics_name",
        ...
        return len(domain) >= 4 and any(
            domain.endswith(compact(suffix)) for suffix in official_suffixes
        )
    if policy == "household_asset":
        return bool(household_asset_taxonomy_terms(claim))
    raise ValueError(f"unsupported metadata recall policy: {policy}")


def merge_table_metadata_recall(
    fused: list[dict[str, Any]], hits: list[Mapping[str, Any]],
    lookup: Mapping[tuple[str, str], Mapping[str, Any]],
) -> list[dict[str, Any]]:
    """Merge label-free official table metadata FTS hits into Stage A."""
    ...
    for hit in hits:
        ...
        row = by_key.setdefault(key, {
            "table": dict(lookup[key]), "org_id": key[0], "tbl_id": key[1],
            "lexical_rank": None, "lexical_score": None,
            "dense_rank": None, "dense_score": None, "rrf_score": 0.0,
        })
        rank = int(hit.get("rank") or 10**9)
        row["metadata_recall_rank"] = rank
        row["metadata_recall_score"] = float(hit.get("score") or 0.0)
        row["metadata_recall_term_count"] = int(hit.get("matched_term_count") or 1)
        row["metadata_recall_term_order"] = int(hit.get("first_term_order") or 10**9)
...
def period_after_metadata_end(period: Any, prd_se: Any, range_text: Any) -> bool:
    """Return true only when the request is newer than a parseable range end.

    This is the narrow stale-snapshot case. Requests older than the published
    range remain hard mismatches and are never restored.
    """
    requested_type = normalize_prd_se(prd_se)
    requested_digits = re.sub(r"\D", "", str(period or ""))
    text = str(range_text or "").strip()
    if requested_type == "M" and len(requested_digits) >= 6:
        ...
        return int(requested_digits[:6]) > end
    if requested_type == "Y" and len(requested_digits) >= 4:
        years = [int(value) for value in re.findall(r"(?<!\d)(\d{4})(?!\d)", text)]
        return len(years) >= 2 and int(requested_digits[:4]) > years[-1]
    return False
...
def apply_pre_rerank_metadata(
    ...
    *, minimum_candidates: int,
    ...
    requested = requested_claim_periodicity(claim)
    exact: list[dict[str, Any]] = []
    unknown: list[dict[str, Any]] = []
    stale_forward: list[dict[str, Any]] = []
    counts = {
        ...
        "period_range_stale_forward_backfill": 0,
    ...
    for candidate in candidates:
        ...
        if requested and periods and requested not in periods:
            ...
            continue
        requested_period = requested_claim_period(claim)
        matching_ranges = [
        ...
        range_mismatch = bool(
            requested_period and known_range_states and not any(known_range_states)
        )
        stale_snapshot = bool(
            range_mismatch and matching_ranges
            and all(period_after_metadata_end(requested_period, requested, value)
                    for value in matching_ranges)
        )
        if range_mismatch:
            counts["period_range_mismatch_rejected"] += 1
        if range_mismatch and not stale_snapshot:
            continue
        enriched = dict(candidate)
        ...
        if stale_snapshot:
            ...
            enriched["pre_rerank_period_range_state"] = "stale_snapshot_forward"
            enriched["metadata_score"] -= 0.30
            enriched["metadata_signals"] = [
                *metadata_signals, "period_range_stale_snapshot_penalty",
            ]
            counts["period_range_stale_forward_backfill"] += 1
            stale_forward.append(enriched)
        elif requested and requested in periods:
            enriched["pre_rerank_period_state"] = "exact"
    ...
    kept = (
        exact if len(exact) >= minimum_candidates
        else [*exact, *unknown, *stale_forward]
    )
    return kept, counts
...
def retrieve_without_reranker(
    ...
    metadata_recall_top_k: int = 0,
    metadata_rerank_slots: int = 0,
    metadata_recall_policy: str = "all",
) -> dict[str, Any]:
    original_claim = claim
    claim = measurement_focused_retrieval_claim(claim)
    ...
    item_hits = (
        postgres_store.item_table_recall(
            ...
            prd_se=requested_claim_periodicity(claim),
    ...
    metadata_enabled = metadata_recall_enabled(claim, metadata_recall_policy)
    metadata_terms = table_metadata_recall_terms(claim) if metadata_enabled else []
    metadata_hits = (
        postgres_store.table_metadata_recall(
            metadata_terms, limit=metadata_recall_top_k,
            prd_se=requested_claim_periodicity(claim),
        )
        if metadata_enabled and metadata_recall_top_k > 0
        and hasattr(postgres_store, "table_metadata_recall") else []
    )
    fused = merge_table_metadata_recall(fused, metadata_hits, lookup)
    ...
    pool = reserve_item_recall_slots(
        pool, fused, metadata_hits,
        top_k=min(rerank_top_k, len(fused)), slots=metadata_rerank_slots,
    )
    ...
    return {
        ...
        "claim_fingerprint": claim_fingerprint(original_claim),
        ...
        "metadata_recall_terms": metadata_terms,
        "metadata_recall_hit_count": len(metadata_hits),
        "metadata_rerank_slots": metadata_rerank_slots,
        "metadata_recall_policy": metadata_recall_policy,
        "metadata_recall_enabled": metadata_enabled,
...
def rerank_table_record(
    ...
    for candidate, score in zip(candidates, scores):
        ...
        ranked.append(row)
    # Preserve every scored retrieval candidate for bounded downstream recall
    # protection. The final table_candidates policy remains unchanged.
    all_scored_candidates = [dict(row) for row in ranked]
        ...
        with store.metadata_read_guard():
            for index, (row_number, claim) in enumerate(pending, 1):
                ...
                packet = retrieve_without_reranker(
                    ...
                    metadata_recall_top_k=args.metadata_recall_top_k,
                    metadata_rerank_slots=args.metadata_rerank_slots,
                    metadata_recall_policy=args.metadata_recall_policy,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--metadata-recall-top-k", type=int, default=0)
    parser.add_argument("--metadata-rerank-slots", type=int, default=0)
    parser.add_argument(
        "--metadata-recall-policy",
        choices=("all", "survey_exact", "household_asset"),
        default="all",
    )
    parser.add_argument(
    ...
    if args.metadata_recall_top_k < 0 or args.metadata_rerank_slots < 0:
        parser.error("metadata recall limits must be non-negative")
    if args.metadata_rerank_slots > args.table_rerank_top_k:
        parser.error("--metadata-rerank-slots cannot exceed --table-rerank-top-k")
...
def main() -> None:
    ...
    base = [
        ...
        "--metadata-recall-top-k", str(args.metadata_recall_top_k),
        "--metadata-rerank-slots", str(args.metadata_rerank_slots),
        "--metadata-recall-policy", args.metadata_recall_policy,
