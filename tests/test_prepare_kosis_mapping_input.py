...
def test_forecast_value_is_rejected_as_not_observed():
    out = normalize_row(
        measurement_row(
            claim_text="보고서는 연간 수출이 6685억달러가 될 것으로 예상했다.",
            measurement_indicator="연간 수출 총액",
            value="668500000000",
            unit="달러",
...
def test_vague_this_year_without_explicit_period_is_not_defaulted_to_article_year():
    out = normalize_row(
        measurement_row(
            date="2025-01-01",
            claim_text="올해 들어 수출액은 높은 수준이었다.",
            measurement_period="-",
            measurement_prd_se="-",
        )
    )
